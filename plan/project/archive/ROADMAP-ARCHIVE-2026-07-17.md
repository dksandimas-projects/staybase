...
> Full snapshot of `plan/project/ROADMAP.md` as of 2026-07-17, taken before the roadmap was compacted.
> Do NOT load this file during normal implementation tasks. The active roadmap is `plan/project/ROADMAP.md`.
> Completed-phase diaries, shipped audit-fix batches, and closed findings live here (and in Git history).
...
- ✅ **BDUX-04 — Progressive disclosure rules.** Completed registration shows compact summary card with optional Edit; nightly rate breakdown already collapsible (`<details>`); payment/refund history and incidental charges auto-collapse when 4+ entries; breakfast selections collapsible with recorded-count badge; email actions collapsed by default behind "Resend email to ..." disclosure.
...
- ✅ **BDUX-08 — Component extraction and regression coverage.** Workspace header, tabs/panels, readiness card, and action footer in `BookingDrawerWorkspace.tsx`; registration form extracted to `BookingRegistrationForm.tsx`; email actions extracted to `BookingEmailActions.tsx`; incidental charges extracted to `IncidentalChargeList.tsx`. Three new extracted components (435 lines), reducing `BookingsPage.tsx` by 284 lines to 5,007. All 839 source-pattern tests updated and passing. Authenticated visual QA deferred as post-cutover item.
...
- ✅ **BDUX-05g — Folio interaction coverage.** 27 source-pattern tests covering button visibility by status/role/ledger state, focus trap, mobile layout, validation, duplicate-submit protection, success/error outcomes, applied-voucher replacement state, checked-out payment/refund behavior. All passing (committed as `phase-12-booking-drawer-folio-interaction.test.ts`).
...
- ✅ **BDUX-05k — Two-column Folio workspace.** Desktop uses independent wide ledger/history and sticky financial-summary columns so a tall summary cannot reserve whitespace between ledger cards; mobile preserves the workflow as a single column without horizontal overflow.
...
- ✅ **ETR-S02a — Environment-aware Admin routing.** Canonical staging Admin hosts route all server actions to the staging guest/API host even when a production fallback URL is present. Production Settings shows staging reset as unavailable and links to staging instead of presenting a button that can only fail.
...
- ✅ **ETR-S09 — Single reset job and safe recovery.** Acquire an atomic project-scoped lock before deletion, return the existing job for duplicate/concurrent requests, persist phase checkpoints outside collections being cleared, and support bounded stale-lock recovery plus idempotent resume after timeout/restart.
- ✅ **ETR-S10 — Preview-bound execution.** Persist a short-lived preview manifest ID/hash containing project, scope, counts, affected rooms, and preservation decisions. Execute requires that identifier and rejects expired, mismatched, or materially drifted previews with instructions to preview again.
- ✅ **ETR-S11 — Fail-closed completion semantics.** Any item/phase failure marks the job `incomplete` or `failed`, returns a non-success response, suppresses the completion toast, retains failure detail/checkpoints, and exposes retry/resume. A `200 success` is allowed only after deletion and integrity verification both pass.
...
## Phase 12 shipped blocks — archived 2026-07-31
> Moved out of `plan/project/ROADMAP.md` per its own rule ("when a phase or fix batch fully ships, move its detail to the archive and keep a one-line ✅ status here"). Every block below was fully shipped; the roadmap retains a one-line pointer. Open follow-ups (BSP-03 manual QA, HSD-05 real-device QA) were left in the roadmap and are NOT here.
