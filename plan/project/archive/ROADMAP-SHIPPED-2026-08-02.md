...
## MRB-08 — Corporate `/corporate/book`: multi-room

- ✅ **MRB-08 — Corporate `/corporate/book`: multi-room** — Shipped 2026-08-02 at v0.232.0 (merge commits `5059eb6` + `f927b49`, decision #167). Server: per-stay negotiated rate lookup from `corporateCodes/{code}.ratePerRoomType[stay.roomType]` (then per-type flat `corporateRate`, then standard `pricePerNight`); the create transaction increments `usageCount` by `assignedRooms.length` and the cap check accepts `requestedUses`, so an over-cap block is rejected before any write; cancellation decrements per child (MRB-13 will consolidate the decrement into a single transaction via the scope selector). Client: `/corporate/book` mirrors `BookingPage`'s MRB-06 room cart (per-stay type + occupancy + own extra beds, claimed rooms filtered out, submission blocked until every stay is creatable), auto-distributes page totals via `rebalanceGuestDistribution`, shows per-stay + aggregate totals, and sends `roomSelections[]` to the existing `/api/bookings/create` endpoint. N=1 byte-equivalent. Helper test + end-to-end source-text test in `guest-app/tests/api/mrb-08-corporate-multi-room.test.ts`.
