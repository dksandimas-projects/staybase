...
- ⬜ **QA-12 — Incoming call should pop up on every admin page, not just Intercom Inbox.** Confirmed in code: the incoming-call banner (`incomingCall` state, ringing/active UI with answer/decline controls) only renders inside `IntercomInboxPage.tsx` (lines ~322+). If front desk/admin is on Bookings, Rooms, Reports, etc. when a guest calls, there's currently no visible indicator or way to answer without first navigating to Intercom — they'd have to rely on the ringtone (QA-10) alone. Feature request: hoist a compact incoming-call popup/toast (room number, guest name, Answer/Decline) into `AdminLayout.tsx` (or another app-shell-level component) so it's visible and actionable from any admin page, mirroring the existing `incomingCall` Firestore state already wired in `AdminContext.tsx`.
- ⬜ **QA-13 — Incoming Intercom *message* notification sound should be more emphasized.** Confirmed in code: `IntercomInboxPage.tsx`'s `playNotificationSound()` plays the admin-configured `notificationSoundUrl` clip exactly once per new unread guest message, and only fires when the Inbox tab isn't focused (`!isInboxFocused`) and sound isn't muted. It's a single one-shot playback — no repeat, no loudness/urgency escalation — unlike the call ringtone (QA-10), which loops every 3 seconds with a dual-oscillator warble until answered. Feature request: make the message notification more noticeable/emphasized (e.g. repeat a few times if unread and still unfocused, add a stronger/longer default tone, or escalate volume/cadence the longer a message sits unread), plus consider pairing it with the QA-12 global popup so unread messages are hard to miss from any admin page, not just audio alone.
...
- ⬜ **QA-17 — Guest Intercom should remember the guest's name across reloads instead of re-asking every time.** Confirmed in code: `IntercomPage.tsx` initializes `showNamePrompt` to `true` on every mount (`useState<boolean>(true)`), and `guestName`/`nameInput` are plain component state set only via `handleNameSubmit()` — nothing persists the name anywhere. `localStorage` is used elsewhere in this same file, but only for the message rate-limit counter (`intercomRate:{roomNumber}`), not the guest's name. So refreshing the page, or reopening the QR link later in the same stay, always re-prompts for the name from scratch. Feature request: persist the entered name to `localStorage` (scoped per room, e.g. `intercomGuestName:{roomId}`) once submitted, and skip `showNamePrompt` on mount if a name is already stored for that room — matching the "remembered once per stay" experience a native app would have.
