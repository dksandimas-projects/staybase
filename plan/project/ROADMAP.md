...
> Last updated: July 14, 2026 (Finance Lifecycle Recommendations queued — 5 post-remediation follow-ups FLR-01..FLR-05 from `AUDIT-FINANCE-LIFECYCLE-2026-07-12.md §Post-remediation recommendations`, added after verifying all 20 FL fixes merged with the full suite green: FLR-01 historical data repair script for pre-fix cohorts (FL-02/FL-03 wrong totals, pre-FL-05 tenderless store orders), FLR-02 `bookings` update-rule field allowlist (staff clients can still write pricing fields directly), FLR-03 bounded ledger listeners deferred with an explicit ~1-year trigger, FLR-04 shared finance invariant test assertions, FLR-05 owner handover items (annotate pre-FL-05 Daily Closes, accountant VAT confirmation, staging money-path walkthrough). Earlier: Finance Lifecycle polish batch completed on `fix/finance-lifecycle-polish`: consistent pre-discount totals, rules-enforced charge safeguards, idempotent onsite payments, and correct receipt discount attribution. All 20 findings are fixed.)
...
- ⬜ **FLR-01 — Historical finance data repair (one-off integrity scan)** — pre-2026-07-13 documents still carry wrong money: FL-02 cohort (`totalPrice` overcharges by `pointsRedeemedValue`), FL-03 cohort (zeroed legacy bookings), and pre-FL-05 delivered direct-paid store orders with no tender entry (historical Outstanding keeps the phantom gap). Build `scripts/finance-integrity-scan.ts` (Admin SDK): flag `rateBreakdown.finalTotal !== totalPrice`, zero totals with non-empty room lines, non-finite totals, tenderless direct-paid orders → review CSV first, corrective writes only after staff sign-off (append-only on the ledger side).
- ⬜ **FLR-02 — `bookings` update rule field allowlist** — `firestore.rules` still allows any staff client to write `totalPrice`/`pointsAwarded`/`status` directly, bypassing every server-authoritative API the FL batches route through; the largest remaining finance-integrity hole. Enumerate legitimate client-side writes (registration, guest ID photo, breakfast, notes, the `payment-confirmed` status write in `AdminContext`) and enforce `diff().affectedKeys().hasOnly([...])` (pattern already used on `members`), or move the status write behind an API first and exclude `status` + all pricing fields.
...
- ⬜ **FLR-04 — Shared finance invariant assertions in tests** — every FL SEV-1/2 was two money representations drifting; per-fix regressions pin the bugs, nothing guards the class. Add a shared helper (breakdown sums to `finalTotal`, `finalTotal === totalPrice`, amounts finite, streams disjoint) and call it from every handler test that writes pricing.
- ⬜ **FLR-05 — Operational handover items** *(owner-facing, no code)* — (a) annotate pre-FL-05 Daily Closes in the handover notes (their store-cash variances are now explained; close docs are locked, don't edit); (b) confirm the FIN-06 client-side-VAT posture with the accountant before the first BIR filing period; (c) one staging browser walkthrough of the full money path (book → pay → charge → store order both tender types → checkout → Daily Close → exports) with the owner before the next `dev → main` milestone merge (`feat:` merge per pre-launch versioning rule).
...
| Finance Lifecycle Recommendations (July 14) | 5 | 0 | 5 (FLR-01..FLR-05 open — 2 actionable, 1 deferred with trigger, 1 test hardening, 1 owner handover — see audit doc §Post-remediation recommendations) |
