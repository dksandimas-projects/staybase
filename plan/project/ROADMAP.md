...
> Last updated: August 3, 2026 (CRL-07 completed on `feature/phase-12-crl-07-refund-liability`; running order — CRL-08/09).
...
| 12 — Enhancements, Multi-Room & CRL | 🔄 Active (MRB-01..13 & CRL-01..07 shipped) | Open MRB-11/12/14/15 & CRL-08/09 items (§Phase 12 below) |
...
- ✅ **CRL-07 — Reservation refund liability + admin workflow (WITH MRB-04/13)** — shipped 2026-08-03 on `feature/phase-12-crl-07-refund-liability` (decision #173). The destructive cancel atomically materialises a `cancellationLiability` snapshot onto the cancelled entity (reservation header for reservation-scope cancels + new-path N=1; booking doc for per-child + legacy null-`reservationId` cancels). `policyResult` is read-only post-cancel; `approvedAmount` defaults to `policyResult.policyRefund` and is reduced only via a new admin-only `POST /api/bookings/cancellation-exception` (reason ≤500 chars, amount bounded by `policyRefund`, idempotent on amount+reason). The existing admin-only `POST /api/bookings/add-refund` continues to record processed refunds; a new read-only `POST /api/bookings/cancellation-liability` projection reads the stored snapshot + the cumulative refunds subcollection sum and returns one of five derived states (`not-required` / `retained` / `pending-processing` / `partially-processed` / `processed`) via the pure `computeCancellationLiabilityState` helper. Front-desk staff can cancel + see the panel; only admins can record a refund or apply an exception. Admin UI: `CancellationLiabilityPanel` (mounted in the drawer) + `CancellationExceptionModal` (admin-only, requires reason). Source-text + state-helper tests in `guest-app/tests/api/crl-07-refund-liability.test.ts` (24) + `shared/__tests__/crl-07-liability-state.test.ts` (23). Full spec in `plan/project/archive/ROADMAP-SHIPPED-2026-08-03.md` + `plan/docs/DECISIONS-FEATURES.md #173`. End-to-end + staging rehearsal follow in CRL-09.
