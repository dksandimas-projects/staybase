...
- ⬜ **FR-05 (deferred, efficiency not correctness)** — the payments/charges `collectionGroup` listeners re-subscribe on every `bookings` change (dep `[bookings]`), re-reading the full set each time → avoidable Firestore reads on the client's Blaze plan. Keep listeners stable and join booking refs in a memo. Left for a follow-up; does not affect displayed figures.
