...
> Last updated: July 9, 2026 (Phase 12 fix queue prioritized by estimated effort: smallest fixes first)
...
- ⬜ **P1 / Small effort — Senior/PWD discount ID upload repair** — fix Step 3 OSCA/PWD image upload in the guest booking flow. Initial code check: upload writes directly to Firebase Storage at `bookings/{bookingId}/discount-id/{filename}` using the preallocated booking ID; Storage rules currently allow public write and staff-only read, so the likely failure is in client upload/error handling or URL propagation rather than read permissions. The current handler shows only an `alert()` on failure, uses the compressed file name directly in the storage path, does not reset the file input after success/delete, and booking creation rejects the discount when `discountIdPhotoUrl` is missing. Fix should add field-level upload error/retry state, sanitize filenames, reset the file input on success/delete, keep upload state scoped to the discount ID field, preserve `discountIdPhotoUrl` in `/api/bookings/create`, and add regression/manual QA coverage for upload success, failed retry, same-file reselect after delete, and server rejection when `discountType` is set without an uploaded ID URL.
- ⬜ **P2 / Medium effort — Voucher application repair** — investigate and fix vouchers not applying in the guest booking page. Initial code check: Step 3 validation calls `/api/validate/voucher` with Turnstile, so failed/missing tokens can make the Apply action fail before discount state is set; client percent-voucher math uses `subtotal` while server comments/documentation say vouchers apply after Senior/PWD discount, so the preview and persisted `voucherDiscount` can drift; booking creation revalidates and can drop/reject stale vouchers. Fix should make Apply feedback explicit, keep client/server discount math in one shared helper, persist the canonical `voucherCode` returned by validation, add regression tests for voucher-only and Senior/PWD+voucher bookings, and verify `usageCount` increments only after successful booking creation.
- ⬜ **P3 / Medium effort — Check-in gate repair** — require payment/booking eligibility plus saved guest ID photo and required guest registration fields before staff can check in a guest. Server must enforce the same rule in `/api/bookings/checkin`, and the admin drawer must show a missing-items checklist instead of allowing a premature check-in. Also resolve the current `payment-confirmed → confirmed` mismatch. Full plan: `plan/features/BOOKINGS-MANAGEMENT.md §Implementation Plan — Check-In Gate`.
- ⬜ **P4 / Large effort — PDF generation repair** — fix admin booking PDFs and report/QR print-to-PDF reliability. Scope from 2026-07-08 review: open PDF tabs synchronously or provide `pdf.save()` fallback so popup blockers do not make buttons appear broken; replace or add known-good jsPDF-compatible TTF font assets (`Inter-Regular.ttf` is currently referenced but missing, and `APOLLO.otf` triggers a jsPDF font encoding error); detect uploaded ID image MIME type instead of forcing every `addImage` call to `"JPEG"`; wrap PDF builders in user-visible error handling/toasts; decide whether Reports needs real jsPDF/html2canvas export or clearer "Print" copy because the current "Print / Save PDF" button only calls `window.print()`.
- ⬜ **P5 / Large effort — Guest-facing price breakdown for mixed regular/weekend/holiday rates** — show itemized rate lines in booking Step 1/3/4, persist `Booking.rateBreakdown`, and reuse it in `/my-booking`, emails, and admin receipts so guests can understand why the total is higher or lower than expected. Full plan: `plan/features/BOOKING-FLOW.md §Implementation Plan — Guest Price Breakdown`.
...
| 12 — Post-Launch | 16 | 8 | 5 not started (guest price breakdown, PDF generation repair, check-in gate repair, voucher application repair, Senior/PWD ID upload repair) + 3 deferred |
