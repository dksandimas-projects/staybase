...
- 🔄 **PC-02 — Provision `spark-inn-prod`** — rules + 6 indexes + storage rules **deployed and verified matching repo** 2026-07-14; Storage CORS applied; Vercel env vars set in both scopes; **region confirmed US-EAST1**, Firestore/Storage enabled, and **browser API key restricted to production domains**. Remaining (console): budget alert, Auth providers (Email/Password + Google), authorized domains, **Firestore backups/PITR** (non-negotiable for the finance ledger).
