...
- ⬜ **QA-09 — No way to end a call on mobile in the guest Intercom view.** `IntercomPage.tsx`'s active-call overlay (`callState !== "idle"`) is `absolute inset-0` inside a container that uses `min-h-screen` (100vh) with `overflow-hidden` and `justify-between` to pin the red "End Call" button (`handleEndCall`, `PhoneOff` icon) to the bottom via `pb-8`. On mobile browsers, 100vh does not account for the dynamic address bar / home-indicator safe area, so the actual visible viewport can be shorter than the container's computed height — pushing the bottom controls below the fold with `overflow-hidden` blocking any scroll to reach them. Other guest-app/admin-app components (`BottomTabBar.tsx`, `Drawer.tsx`, `Modal.tsx`, `Toast.tsx`, `AdminLayout.tsx`) already use `dvh` units / `env(safe-area-inset-bottom)` for this exact class of problem — `IntercomPage.tsx` doesn't. Likely fix: swap `min-h-screen` for `h-dvh` (or add safe-area padding) on the call overlay and its containers.
...
- ⬜ **QA-11 — Guest Intercom view scrolls as a whole page instead of only the message list.** Per the attached screenshot, the guest has to scroll the entire page to reach the Quick Requests row and the message composer at the bottom. Root cause: `IntercomPage.tsx`'s outer `<main>` uses `min-h-screen` (a *minimum*, not a fixed height) around a `flex flex-col` layout; as the message thread grows, the whole container grows past one screen instead of clipping, so the composer and quick-request chips get pushed off-screen and the browser scrolls the full document rather than just the inner `.flex-1.overflow-y-auto` message pane. Likely fix: change the outer container to a fixed `h-dvh` (not `min-h-screen`), keeping header, quick requests, and composer pinned, with only the message list (already `overflow-y-auto`) scrolling internally.
- ⬜ **QA-12 — Incoming call *and* incoming message should pop up on every admin page, not just Intercom Inbox.** Confirmed in code: the incoming-call banner (`incomingCall` state, ringing/active UI with answer/decline controls) only renders inside `IntercomInboxPage.tsx` (lines ~322+). If front desk/admin is on Bookings, Rooms, Reports, etc. when a guest calls, there's currently no visible indicator or way to answer without first navigating to Intercom — they'd have to rely on the ringtone (QA-10) alone. **Expanded scope (owner request):** the same gap exists for new chat messages, not just calls — `AdminContext.tsx` already computes a global `unreadIntercomCount` (`useMemo` over the `intercoms` state, all rooms, all pages) and the Dashboard shows it as a static badge, but nothing surfaces an actual popup/toast for a *new* incoming message while staff are elsewhere in the app; they'd only notice via the notification sound (QA-13) or by happening to check the badge. Feature request: hoist two compact popups/toasts into `AdminLayout.tsx` (or another app-shell-level component) — (1) incoming call (room number, guest name, Answer/Decline), reusing the existing `incomingCall` state, and (2) incoming message (room number, guest name, message preview, a "View" action that jumps to that thread in Intercom Inbox), reusing the existing `intercoms`/`unreadIntercomCount` state — so both are visible and actionable from any admin page, not just Intercom Inbox.
- ⬜ **QA-13 — Incoming Intercom *message* notification sound should be more emphasized.** Confirmed in code: `IntercomInboxPage.tsx`'s `playNotificationSound()` plays the admin-configured `notificationSoundUrl` clip exactly once per new unread guest message, and only fires when the Inbox tab isn't focused (`!isInboxFocused`) and sound isn't muted. It's a single one-shot playback — no repeat, no loudness/urgency escalation — unlike the call ringtone (QA-10), which loops every 3 seconds with a dual-oscillator warble until answered. Feature request: make the message notification more noticeable/emphasized (e.g. repeat a few times if unread and still unfocused, add a stronger/longer default tone, or escalate volume/cadence the longer a message sits unread). Now directly paired with `QA-12`'s message popup (no longer just a "consider") so a new message is both heard and visibly shown from any admin page, not audio alone.
- ⬜ **QA-14 — Mobile pages appear zoomed in by default, and the keyboard triggers further zoom on text input focus.** Root cause confirmed in code: Tailwind's Preflight base styles set `input`/`select`/`textarea` to `font-size: 100%` (inherit from parent) rather than a fixed size. Across both apps, form fields are consistently wrapped in a `<label className="... text-sm ...">` (14px) or similar small-text ancestor — e.g. `DateRangePicker.tsx` (check-in/check-out), `BookingPage.tsx`'s shared `TextField` component, and the same pattern repeats in admin forms — so the *effective rendered font-size of the input itself* is 14px, under the 16px threshold. iOS Safari (and some Android browsers) auto-zoom the viewport when a user focuses any text input rendered below 16px, to keep the text legible, and that zoom can persist across navigation within the same mobile session/PWA tab, which is likely what reads as "zoomed in by default on page load" too. Viewport meta tags (`guest-app/index.html`, `admin-app/index.html`) are already correctly configured with `width=device-width, initial-scale=1.0` — the tag itself isn't the problem. Recommended fix: ensure every `input`/`select`/`textarea` renders at a minimum effective font-size of 16px on small viewports (e.g. a Tailwind base override or a `text-base` floor applied directly to form controls, not just their label wrappers) rather than disabling `user-scalable` — forcing `user-scalable=no` would silence the symptom but break pinch-zoom for users who need it (WCAG 1.4.4 / accessibility commitment in `LEGAL.md`).
- ⬜ **QA-15 — Show guest / check-in details inside the Intercom thread so front desk knows who they're talking to.** Confirmed in code: `IntercomInboxPage.tsx` and `IntercomChatPanel.tsx` key everything purely off `roomNumber` — the thread list, chat header ("Room {roomNumber}"), and message composer never reference the occupying guest's name, booking reference, or stay dates. Front desk currently has to separately open the Bookings page and search/filter by room number to figure out who they're actually replying to and what their booking looks like. Feature request: surface the current occupant's booking summary (guest name, booking ref, check-in/check-out dates, status, any special requests/discount flags) directly in the Intercom chat header or a side panel — cross-referencing the `bookings` collection by `roomNumber` for the active/occupied stay — so front desk can address the guest correctly and see booking context without leaving the Intercom view. Consider a link/shortcut into the full booking detail drawer (`BookingsPage.tsx`) for anything not worth duplicating inline.
- ⬜ **QA-16 — QR Management: "Unable to download the QR image. Please try again."** Confirmed exact trigger in code: `QRManagementPage.tsx`'s `handleDownloadPng()` renders the QR as SVG markup (`getQrMarkup()` via `renderToStaticMarkup` on `qrcode.react`'s `<QRCodeSVG>`), wraps it in a Blob, loads it into an in-memory `Image()` via `URL.createObjectURL`, then draws it to a `<canvas>` to export as PNG. The error is set from `image.onerror`, meaning the browser failed to decode/load the SVG blob into the `Image` element — the canvas/PNG conversion step is never reached. This is a known class of cross-browser flakiness (particularly Safari/iOS and some in-app/mobile browsers are inconsistent loading SVG blob URLs into `<img>`/`Image()`, sometimes needing explicit `width`/`height` on the root `<svg>`, not just `viewBox`, or failing silently depending on timing). Needs on-device reproduction (which browser/OS) before a fix; likely remedies include verifying the generated SVG has explicit pixel `width`/`height` attributes, or switching the PNG export path to a canvas-native QR draw (e.g. `qrcode` npm package's canvas renderer) instead of round-tripping through an SVG blob + `Image()`.
- ⬜ **QA-17 — Guest Intercom should remember the guest's name across reloads instead of re-asking every time.** Confirmed in code: `IntercomPage.tsx` initializes `showNamePrompt` to `true` on every mount (`useState<boolean>(true)`), and `guestName`/`nameInput` are plain component state set only via `handleNameSubmit()` — nothing persists the name anywhere. **Fully replaced by `QA-20`**, not just superseded: the free-text "What is your name?" field this item was about is being removed entirely, not just made persistent — see `QA-20` Layer 2.
- ⬜ **QA-18 — Room transfer / room-type upgrade end-to-end workflow.** Full spec added to `plan/features/BOOKINGS-MANAGEMENT.md §Implementation Plan — Room Transfer & Upgrade`. Investigated whether the platform supports a guest moving rooms or upgrading room type: the core mechanism already exists and is more capable than expected — `POST /api/bookings/reschedule` (`handleRescheduleBooking`) lets staff move a booking to a room of any type, fully re-prices against the target room type (base/weekend/seasonal/corporate rate + discounts/voucher/member discount), checks capacity and date conflicts, records a `deltaTotalPrice` in `rescheduleHistory`, and emails the guest — reachable today from the `/calendar` booking drawer's "Move booking" form (any room, any type, not just same-type swaps). Gaps confirmed and queued in the spec: no entry point from the main `/bookings` table/drawer (Calendar-only today); `rooms/{roomId}.status` is never synced on transfer, so the Dashboard room grid can show stale occupancy for an in-house guest's old/new room; no guest-initiated request path (no Intercom quick-request chip for "request room change," ties into `QA-15`); no explicit "collect the price difference" prompt tied to the Additional Payments panel; no distinct in-house-guest handling (housekeeping/turnover note, folio carryover) versus rescheduling a future booking.
- ⬜ **QA-19 — Number of guests text field icon overlaps input border.** Confirmed in code: `TextField` in both `BookingPage.tsx` and `CorporateBookingPage.tsx` wraps the input and icon in an inline `span` wrapper (`<span className="relative">`). Because `span` is an inline element by default, the absolute positioned icon (`top-1/2 -translate-y-1/2`) aligns relative to the inline text baseline rather than the input's actual height, causing the `Users` icon to sag and overlap the lower-left border. Fix: change the wrapper span's className to `relative block` or `relative inline-block w-full`.
- ⬜ **QA-20 — Guest Intercom access guard for checked-out / wrong-guest access.** Raised by owner: a bookmarked/screenshotted `/intercom/:roomId` link keeps working after checkout, and — worse, confirmed during design discussion — once a *new* guest checks into the same room, that same stale link would route the old guest straight into the new guest's live, active conversation (a static room-only link can't distinguish who's asking, only whether the room is currently occupied). Decided approach, keeping the QR code static/print-once (no reprint-per-turnover workflow): full spec in `plan/features/INTERCOM-GUEST.md §Implementation Plan — Current-Guest Access Guard`. 3 layers: (1) gate on `rooms/{roomId}.status === "occupied"` — blocks the vacant-gap case, zero friction; (2) the old free-text "What is your name?" field is removed and replaced entirely by a single **"Last name"** field with a hint underneath — *"This should match the last name used when the room was booked"* — submitted to a **new** `POST /api/intercom/verify-guest` server endpoint, which compares it (Admin SDK, never client-readable since `bookings` is staff-only read) against the room's current `checked-in` booking's `guestDetails.lastName` using a **normalized comparison** — lowercase, trimmed, whitespace-collapsed, punctuation/symbols stripped on both sides — so case (all-caps, all-lowercase) and stray apostrophes/hyphens/periods don't cause false rejections; returns only a boolean, never the real name; on success the verified last name doubles as the guest's display name (used in the greeting and shown in the admin Inbox), and a verified flag + name are stored in `localStorage` keyed by `bookingId` (not room number) so the guest isn't re-asked for the rest of their stay, but the next guest/booking in that room gets a fresh prompt — this fully replaces `QA-17`, which is now obsolete rather than merged; (3) scope `intercoms` message history to a `currentStayId` stamped at check-in so old messages don't linger visually even for a verified new guest. Layer 2 is the one that actually stops a wrong person from reaching an active conversation; layers 1 and 3 are cheap complements.
- ⬜ **QA-21 — Guest Intercom header/tabs UI polish.** Raised by owner from a screenshot. Full detail in `plan/features/INTERCOM-GUEST.md §Implementation Plan — Header & Tab UI Polish`. Two confirmed issues in `IntercomPage.tsx`'s header: (1) the round room-avatar badge renders the raw `roomId` route param directly (e.g. `room-301`, a full slug) inside a 36px circle — too long to fit, so it wraps and spills outside the circle exactly as shown in the screenshot; should show the short resolved room number or a fixed icon instead of the raw slug. (2) The "Chat Support" and "Spark Essentials" tabs are visually underweighted relative to the bold dark header above them — 12px text, 14px icons, thin padding, with only a 2px underline marking the active tab — even though they're the two primary destinations on the screen. Feature request: make both tabs bigger and heavier (larger text/icons, more padding) and consider a filled/pill active state instead of just an underline, so they read as prominent primary navigation rather than an afterthought.
- ⬜ **QA-22 — Dashboard "Today's Breakfast Prep" section.** Feature request (owner, 2026-07-09): front desk should have a guided daily view of who ordered breakfast and whether it's been served yet, on the Dashboard — not buried in Reports. Full spec added to `plan/features/DASHBOARD-OVERVIEW.md §Implementation Plan — Today's Breakfast Prep`. Confirmed the underlying aggregation already exists — `ReportsPage.tsx`'s `dailyKitchenPrep` (lines ~306-331) walks every breakfast booking's `breakfastSelections` map (`yyyy-mm-dd-guest-n` → silog item name) into per-date item counts — but it's a static count report for kitchen quantity planning, not scoped to "today," and has no served/not-served state at all. New Dashboard section: one row per guest with a breakfast order today (room, guest name, item, Served toggle), an unserved-count badge, "All served" complete state, hidden entirely when nobody has breakfast today. Requires a small, non-breaking schema addition — a sibling `breakfastServed?: Record<string, boolean>` map on the booking using the same key format as `breakfastSelections`, so existing exports/reports/PDFs relying on the current field are untouched.
- ⬜ **QA-23 — Per-payment-method "Require reference number" toggle.** Feature request (owner, 2026-07-09). Full spec added to `plan/features/SETTINGS.md §2 Payment Methods` (Add/Edit modal) and `plan/features/BOOKING-FLOW.md` (Step 3 guest-facing field + server validation). Confirmed no reference number field exists anywhere today — `PaymentMethodConfig` (`shared/types/index.ts`) has no such field, and neither `BookingPage.tsx` nor `handleCreateBooking` reference one. New `requireReferenceNumber?: boolean` field on each payment method in Settings → Payment Methods, **defaulting to `true` when missing** (no migration needed, matches the existing surface-pill default pattern already used for `showInStore`/`showInCorporate`). When true for the guest's selected method, Step 3 shows a required "Reference Number" text field next to the payment screenshot upload; when false (e.g. admin turns it off for a specific method), the field is hidden and not required. Stored as `paymentReferenceNumber` on the booking, re-validated server-side in `/api/bookings/create` (never trust a client-side skip). **Important distinction:** `requireReferenceNumber` only gates the *guest*-facing requirement at booking time — it never limits staff. The booking drawer gets its own always-editable Reference Number field next to the payment proof thumbnail (`plan/features/BOOKINGS-MANAGEMENT.md §Reference Number field`), so front desk can add or correct it manually when confirming payment, even for a method the guest wasn't required to submit one for. **Also queued:** add `paymentReferenceNumber` to the Reports exports — confirmed today it's missing from both the admin-only Full Backup XLSX "Bookings" sheet (`ReportsPage.tsx`'s `bookingRows`, ~line 403-441, which already includes `Payment Method` right next to where this belongs) and the Sales XLSX "Bookings" sheet (`bookingsHeaders`/`bookingsRows`, ~line 559-570, same situation). Full column-table spec updated in `plan/features/REPORTS.md §Sheet: Bookings` and `§Sales Detail Table`. **Also included:** the simple "Export CSV" button in the Reports page header (`handleExportCSV`, ~line 358-368, visible on both tabs) — it doesn't carry Payment Method at all today, so both Payment Method and Reference Number are added together there (`plan/features/REPORTS.md §Export CSV Button`), so the reference number has context for which method it belongs to. All three export surfaces (Full Backup XLSX, Sales XLSX, simple CSV) now in scope for this item.
- ⬜ **QA-24 — Custom date range for Reports & Metrics.** Feature request (owner, 2026-07-09): allow picking an arbitrary date range for reports/exports, while keeping the existing preset dropdown. Full spec in `plan/features/REPORTS.md §Custom Date Range`. Confirmed in code: the page-header date selector (`ReportsPage.tsx` `dateRange` state, ~line 77) is a single `<select>` with only three fixed presets (Last 7 Days / Last 30 Days / Last Quarter); `periodStart`/`periodEnd` (~lines 99-110) always compute as "today minus N days" through "today, end of day" — there's no way to pick a specific start/end date or a range not ending today. This gates every chart, table, and export on the page (CSV, Sales XLSX, Full Backup all read from `periodStart`/`periodEnd`). Target: add a fourth **"Custom Range"** option to the existing dropdown (the three presets stay as-is and remain default) — selecting it reveals two native date inputs for start/end, which then drive `periodStart`/`periodEnd` instead of the day-count calculation. Needs basic validation (end ≥ start, a sane max span) and should flow through to export filenames/labels automatically since those already derive from the same two values.
- ⬜ **QA-25 — Wrong logo variant on the dark email header.** Bugfix (owner, 2026-07-09): the spark inn logo in transactional emails is hard to read because it's the wrong color variant for a dark background. Confirmed in code: `guest-app/server/handlers/email.ts`'s `brandLogoUrl()` (~line 76-78) resolves `config.logos.navbar`, and it's rendered inside the email layout's hero block (~line 230) whose background is `config.colors.sidebar` — a dark color. Per `CLAUDE.md` Hard Rules, dark backgrounds should always use `config.logos.white`, not the navbar variant (which is meant for a light/transparent background like the guest-app navbar it's named for). Fix: swap `brandLogoUrl()` to resolve `config.logos.white` instead of `config.logos.navbar`. Full note in `plan/features/EMAIL-PDF-STORAGE.md §Email Content Checklist`. Affects every transactional email since they all share the same layout header.
- ⬜ **QA-26 — Admin Intercom Inbox should auto-scroll to the latest message on load.** Feature request (owner, 2026-07-09). Full note in `plan/features/INTERCOM-INBOX.md`. Confirmed missing in code, not just a visual impression: `admin-app/src/components/IntercomChatPanel.tsx` declares `messagesEndRef` (line 47) and places it at the bottom of the message list (line 179) — the scaffolding for auto-scroll is there — but no `useEffect` anywhere in the file (or in the parent `IntercomInboxPage.tsx`) ever calls `.scrollIntoView()` on it, so the ref does nothing. Staff opening a thread currently land wherever the browser's default scroll happens to be, typically the oldest messages for a long thread, not the latest. Fix: add a `useEffect` keyed on `messages`/`roomNumber` that scrolls to `messagesEndRef` — same pattern the guest-facing `IntercomPage.tsx` already correctly implements for the guest's own chat view (it has a working `messagesEndRef.current?.scrollIntoView({ behavior: "smooth" })`), just missing on the admin side.
