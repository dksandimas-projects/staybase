...
- 🔄 **MRB-11 — Reports use the correct owner for each metric** — proposed 2026-08-03 (decision #177). Header metrics read from `reservations/{id}`; child-line metrics (occupancy, room revenue, ADR) read from `bookings/{id}`; legacy null-`reservationId` = 1 reservation + 1 room stay. New `Booking.revenueAllocation` snapshotted at create + on reschedule; **retires the `splitBookingRevenue` heuristic** (pre-MRB-11 rows tag `"allocation: legacy-heuristic"`). MDs + tests per decision #177.
