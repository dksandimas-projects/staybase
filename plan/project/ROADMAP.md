...
- [ ] `manifest.json` generated by Vite PWA plugin with name, short_name, theme_color, background_color, display ("standalone"), start_url
- [ ] PWA icons (192×192 + 512×512 PNG) added to `guest-app/public/brand/` per AUDIT-16
- [ ] Workbox runtime caching strategy configured — `NetworkFirst` for `/rooms`, `/book`, etc. (always fetch fresh); `CacheFirst` for `/brand/*`, fonts, and other static assets
- [ ] `<meta name="theme-color" content="#EA8A1A">` in `guest-app/index.html`
- [ ] `<link rel="apple-touch-icon" href="/brand/apple-touch-icon.png">` in `guest-app/index.html`
- [ ] Service worker registered on guest-app boot; offline fallback page shown when navigation fails on cached routes
