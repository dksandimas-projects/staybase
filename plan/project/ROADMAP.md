...
### MRB-12 (Admin Reservation Embed) — proposed 2026-08-03
> Reservation #179 reserved. Per decision #159 (MRB foundation) the `Reservation` doc carries every aggregate the desk needs (`totalPrice`, `paymentStatus`, denormalized counters, `aggregateRevenueAllocation` per #177, `cancellationLiability` per #173). Today the `BookingsPage` reservation row at `admin-app/src/pages/BookingsPage.tsx:1711-1721` reads the **filtered in-memory children** for total + balance (line 1701-1705) — the same children the `groupedRows` useMemo scans at `1671-1734` — and the drawer reservation strip at `4347-4378` shows ref + position + sibling buttons but no money. **ADJACENT FINDING — the row's `listReservationBalance` is silently wrong when an active filter hides one of the rooms** (e.g. `brt=` for a different type inside an N>1 group) because the sum skips the hidden child. This is the bug that motivates reading the header.
> **Gates:** MRB-14 (add-room + in-stay date changes — the "This room" / "All N rooms" picker lands here as the template for the add-room form), MRB-15 (byte-equivalence for N=1 and legacy is the template for the cleanup pass), BDUX (multiple verification rows close automatically — see ARE-03). **Rejected:** a new `GET /api/reservations/{id}/summary` endpoint (the pure `getReservationFolioSummary` + an AdminContext `reservations/{id}` listener is the surface; no API change); a separate `ReservationsPage` (out of scope — the Bookings table row IS the affordance); rendering every child inline on the row (the existing Show/Hide rooms expander from #159 already handles that; ARE-02 only adds a count badge); a per-stream revenue card on the row (MRB-11's stored `aggregateRevenueAllocation` is the source, the row reads the aggregate total — Reports owns the per-stream breakdown).

- ⬜ **MRB-12-01 — Reservation header is the source of truth for the table summary** — `groupedRows` reads `Reservation.totalPrice` (already in `Reservation`, line `262`) and `getReservationFolioSummary(reservation, children).balance` (`shared/utils/bookingFolio.ts:364`) for the row's `listReservationBalance` instead of summing filtered children. `useAdmin()` gains a single `subscribeToReservations` listener that keeps the `Reservation[]` map in memory (small, ~14 active at this scale; Reports already pulls the aggregate for MRB-11, so the listener is shared, not new). N=1 + legacy null-`reservationId` paths are byte-equivalent — `listRowKind: "booking"` and no row synthesised. **Fixes the ADJACENT FINDING** by construction: the header doesn't filter, so a hidden child is still in the balance.
- ⬜ **MRB-12-02 — Cancellation-count badge + aggregate-status pill on the row** — the reservation row's Status column (lines `1411-1444`) reads `Reservation.paymentStatus` (from `shared/utils/bookingFolio.ts:457` `mapBookingStatusToReservationPaymentStatus` / `538` `computeReservationAggregatePaymentStatus`) instead of the current "Mixed" fallback. Renders `Awaiting payment ₱X` / `Verified` / `Settled` / `Partial` as the canonical pill. When `Reservation.cancelledRoomCount > 0` an additional small `X cancelled` chip renders below the pill with a tooltip listing the cancelled rooms; legacy N=1 single-row path stays as today (no chip — the existing `StatusBadge` is already per-booking).
- ⬜ **MRB-12-03 — Drawer reservation strip shows the reservation aggregate** — the strip at `4347-4378` adds three pills (Total / Paid / Balance) reading `Reservation.totalPrice` + `getReservationFolioSummary(reservation, children)`. The per-booking `BookingDrawerWorkspaceHeader` (line `4426`) keeps its own per-room money below — the aggregate is **additional**, not a replacement. Hidden when `selectedReservationContext` is null (N=1 / legacy) — the strip already gates on that.
- ⬜ **MRB-12-04 — Action-scope chip on every multi-room action** — `renderActionScope` (line `1762`, defined for the cancel flow in #170) is applied to the Reschedule / Move room / Verify payment / Record payment / Add onsite payment / Apply discount action rows in the drawer. **Reschedule + Move + Verify + Record + Add-ons are per-room only** today (the chip is informational — "This room" — so the desk never wonders). **Apply discount is the one that takes the segmented control** in MRB-12-05. N=1 and legacy paths keep their current action labels (no chip — no other room exists).
- ⬜ **MRB-12-05 — "This room" / "All N rooms" segmented control on the discount form** — mirrors the MRB-13 cancel-modal control (lines `5628-5690`). `scope=room` calls the existing per-booking `applyDiscount` path (byte-equivalent to today). `scope=reservation` iterates `selectedReservationContext.rooms`, calls `applyDiscount` for each child, and best-effort updates `Reservation.discountScopeSnapshot` (DSC-01) so the existing discount-chain math + the reservation header's `discountPct` stay consistent. The API surface does not change (the loop is the same code path the desk would manually run today); a transactional `applyReservationDiscount` is the MRB-14+ follow-up. Rejected: server-side `applyReservationDiscount` in MRB-12 (it's a thin loop over a stable endpoint — adding a new endpoint for MRB-12 is speculative; ship the loop, add the endpoint when MRB-14 needs atomicity).
- ⬜ **MRB-12-06 — Tests + MD sync** — `admin-app/src/__tests__/mrb-12-admin-reservation-affordance.test.ts` (source-text): reservation row reads `Reservation.totalPrice` not child sum; row reads `getReservationFolioSummary` for balance; cancellation chip renders when `cancelledRoomCount > 0` and is absent at zero; aggregate-status pill renders the `paymentStatus` text not "Mixed"; drawer strip shows three money pills; action-scope chip on Reschedule / Move / Verify / Record / Discount; discount form segmented control calls `applyDiscount` for every child when `scope=reservation`; N=1 + legacy null-`reservationId` paths are byte-equivalent. MDs: `plan/features/BOOKINGS-MANAGEMENT.md` (new "Multi-room reservation affordances" subsection + the "summary never requires N children" contract); `plan/docs/BACKEND.md` (the `reservations/{id}` listener in AdminContext + the header-as-source-of-truth invariant for the row summary); `plan/project/ROADMAP.md` (flip MRB-12 to ✅ shipped on merge); `plan/docs/DECISIONS-FEATURES.md #179` (implementation record on merge — reserve now, write at land).
