...
> Last updated: July 16, 2026 (Roadmap — expanded **Environment Test Runs & Controlled Data Reset** with production-to-staging refresh: configuration-only or sanitized operational snapshots for normal use, plus a separate expiring restricted diagnostic clone for exceptional unsanitized investigation rather than a casual checkbox. The plan also includes staging-only operational reset, production test runs/scoped cleanup, and the guarded pre-live reset. Other filtering, booking-drawer, payment, and checkout improvements are documented below.)
...
> **Owner requirement added July 16, 2026:** Admin needs a way to refresh staging from production. Normal staging refresh must not casually expose real guest/payment data. The UI offers safe refresh modes; unsanitized access is a separate exceptional diagnostic workflow with stricter controls and automatic destruction.
...
- ⬜ **ETR-R02 — Safe refresh modes.** Offer **Configuration only** and **Sanitized operational snapshot**. Configuration-only copies approved rooms/types, rates, payment methods, catalog, vouchers/corporate configuration, branding/content, and operational settings. Sanitized snapshot additionally copies the operational graph required for realistic booking/order/finance testing after irreversible redaction.
- ⬜ **ETR-R03 — Sanitization is locked for normal refresh.** In the sanitized mode, show **Sanitize personal and payment data** as checked, required, and non-disableable. Allow separate choices to preserve operational dates, financial values, and statuses because those are needed for workflow/reconciliation testing; these choices never permit PII/payment evidence to pass through.
...
- ⬜ **ETR-R05 — Sensitive content and files.** Remove or synthesize guest/internal free-text content, intercom messages, ID images, signatures, payment proofs, and private production Storage URLs. Public brand/catalog assets may be copied through an approved asset path; sensitive-file test cases use known synthetic fixtures rather than production uploads.
- ⬜ **ETR-R06 — Preserve relational and finance integrity.** Preserve booking/order relationships, room/type links, stay duration, lifecycle state, price/rate breakdowns, discounts, vouchers, points, payments, refunds, incidentals, store quantities/tenders, balances, and report reconciliation totals. Map production staff UIDs to staging-safe actors and replace booking/order references with non-production values while retaining stable joins.
- ⬜ **ETR-R07 — Staging isolation.** Preserve staging Auth/staff and environment-specific secrets/configuration. Force-disable guest email delivery, payment callbacks, production notifications, and other outbound side effects during and after import. Imported documents carry snapshot ID/source/import timestamp metadata and a visible **SANITIZED PRODUCTION SNAPSHOT** badge where operational records are displayed.
- ⬜ **ETR-R08 — Scan before import.** Build the transformed snapshot in an isolated job, never log source PII, and run denylist/schema scans for production emails, phones, ID fields, payment references, private notes/messages, and production Storage URLs before any staging write. Fail closed and quarantine the snapshot if sensitive values or unknown sensitive fields remain.
...
#### Exceptional restricted diagnostic clone

> An exact/unsanitized dataset is not a normal staging refresh option. If a defect genuinely cannot be reproduced with sanitized values, use a separate temporary diagnostic clone whose access and controls meet production standards.

- ⬜ **ETR-D01 — Separate workflow, not a toggle.** Do not expose an ordinary unchecked **Sanitize data** option. **Restricted diagnostic clone** is a distinct action requiring recent Admin reauthentication, written diagnostic purpose, requested lifetime, owner/DPO approval record, and explicit acknowledgement that real production data will be present.
- ⬜ **ETR-D02 — Isolated destination.** Provision/import only into an approved restricted diagnostic project/environment, never everyday staging. Its Firebase/Auth/Storage/network access, staff allowlist, logging, export controls, and secret management must be reviewed to production-equivalent standards before import.
...
- ⬜ **ETR-D04 — Disable side effects.** Force-disable guest/staff transactional emails, payment callbacks, webhooks, production notifications, scheduled jobs, and public access. Display a persistent **REAL PRODUCTION DATA — RESTRICTED** banner and prevent the clone from being mistaken for ordinary staging.
- ⬜ **ETR-D05 — Short TTL and destruction.** Default to a maximum 24-hour lifetime unless a shorter approved window is selected. Automatically lock and destroy the diagnostic data/environment at expiry; provide **Destroy diagnostic clone now** and require a post-destruction verification that Firestore data, Storage objects, temporary exports, keys, and caches are gone.
- ⬜ **ETR-D06 — Access and destruction audit.** Record creation approval, purpose, included scope, authorized users, access timestamps where available, exports, expiry, early-destroy action, completion/failures, and destruction verification in a protected production audit location without duplicating copied PII.
- ⬜ **ETR-D07 — Fail-safe behavior.** If approval, project controls, side-effect suppression, TTL scheduling, or destruction verification cannot be proven, refuse to create the clone. Expiry/destruction failure triggers immediate access lock and a visible security incident requiring manual completion.
- ⬜ **ETR-D08 — Privacy/security coverage.** Test source-to-staging one-way enforcement, normal-mode sanitization lock, deterministic joins, PII/file scanning, outbound suppression, staging staff/config preservation, finance invariants, restricted approval denial, isolated destination checks, TTL lock/destruction, and post-destruction absence.
...
- ⬜ Normal staging refresh never offers a functioning unsanitized toggle and cannot import a snapshot that fails the PII/private-file scan.
- ⬜ Any exceptional unsanitized diagnostic copy is isolated from everyday staging, minimally scoped, approval-bound, access-audited, automatically time-limited, and verifiably destroyed.
