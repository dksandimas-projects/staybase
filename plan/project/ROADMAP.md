...
> Last updated: July 8, 2026 (check-in gate repair added to Phase 12 roadmap; PDF repair and guest price breakdown remain planned)
...
- ⬜ PDF generation repair — fix admin booking PDFs and report/QR print-to-PDF reliability. Scope from 2026-07-08 review: open PDF tabs synchronously or provide `pdf.save()` fallback so popup blockers do not make buttons appear broken; replace or add known-good jsPDF-compatible TTF font assets (`Inter-Regular.ttf` is currently referenced but missing, and `APOLLO.otf` triggers a jsPDF font encoding error); detect uploaded ID image MIME type instead of forcing every `addImage` call to `"JPEG"`; wrap PDF builders in user-visible error handling/toasts; decide whether Reports needs real jsPDF/html2canvas export or clearer "Print" copy because the current "Print / Save PDF" button only calls `window.print()`.
- ⬜ Check-in gate repair — require payment/booking eligibility plus saved guest ID photo and required guest registration fields before staff can check in a guest. Server must enforce the same rule in `/api/bookings/checkin`, and the admin drawer must show a missing-items checklist instead of allowing a premature check-in. Also resolve the current `payment-confirmed → confirmed` mismatch. Full plan: `plan/features/BOOKINGS-MANAGEMENT.md §Implementation Plan — Check-In Gate`.
...
| 12 — Post-Launch | 14 | 8 | 3 not started (guest price breakdown, PDF generation repair, check-in gate repair) + 3 deferred |
