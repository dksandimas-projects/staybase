...
> Last updated: July 28, 2026 (marked Spark Rewards Feature Audit (2026-07-18) as fully closed 2026-07-25 — all 5 sections audited, 9 of 11 findings shipped, 2 deferred with rationale; also marked walk-in booking modal scrollability bug MD as shipped earlier today)
...
- ⏸ **MED-3 — "Different email" reconciliation is unimplemented despite being marked done in the spec** — **Deferred 2026-07-25** (decision #135, branch `fix/spark-rewards-different-email-spec`). When a member's account email (e.g. Google) differs from the email on their earlier **anonymous** booking, `linkBookingsByEmail` (which matches only on the token email) won't link it. The spec's two paths — guest self-service prompt ("We found bookings under a different email…") and front-desk "manual link from Member detail drawer" — were both marked `[x]` but neither is built. **Decision:** keep the build path open (front-desk manual link is the smaller surface, ~2-4h), but explicitly mark the spec lines as Deferred rather than building a quick spec-correction patch. The booking-lookup workaround (`/my-booking` ref + email) covers the guest surface; no data loss. The misleading `[x]` markers are themselves a doc-accuracy nit (LOW-3 of a different audit's family, not this one). `plan/docs/DECISIONS-FEATURES.md #135`.
