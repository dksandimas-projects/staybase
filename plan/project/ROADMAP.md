...
### Post-merge Audit — 2026-07-15/16 commit batch (audited 2026-07-16, fixes to close)

> Audit of the ~40 commits merged 2026-07-15 → 2026-07-16 (Notification Center hardening, PRC payment reference semantics, UCO unpaid checkout, booking drawer UX, FSO filters, ETR test runs + staging reset). Architecture and safety engineering verified sound overall — see "Verified correct" below — but two deploy-affecting defects and a broken guest API test suite were found. All findings verified by running the suites and bisecting; admin-app 847/847 green, typecheck clean, guest API 17/416 failing. Ordered by severity.

**SEV-2 (fix first):**

- ⬜ **AUD-01 — Committed API bundle is stale; the deployed staging reset is missing its last two fixes.** Commits `09ccabc` (delete orphaned subcollection documents during staging reset) and `fb3b2fc` (integrityErrors empty-array fallback for Firestore) changed `guest-app/server/handlers/test-runs.ts` but did **not** rebuild `guest-app/api/[...route].js` — violating the committed-bundle rule (GOTCHAS §npm Workspaces, learned from the 2026-07-01 outage). Verified: the bundle has zero occurrences of the orphan-deletion helper and still carries the pre-`fb3b2fc` `undefined` fallback, which makes Firestore reject the cleanup-audit write — so the **deployed** staging-reset execute fails with a 500 even when deletion succeeds, and orphaned subcollection docs are not swept. **Fix:** run the guest-app `build:api` script, verify the regenerated bundle contains both fixes, commit. **Prevention (required, second occurrence of this gap):** add a pre-commit hook or CI check that fails when `guest-app/server/**` or `shared/**` changes without a matching bundle rebuild.
- ⬜ **AUD-02 — Production test-run cleanup orphans booking `charges`, which then contaminate revenue reports.** `handleDeleteTestRun` deletes booking subcollections `payments` / `incidentalCharges` / `notifications` / `audit` — but the real incidental-charge subcollection is named **`charges`** (written by the server checkout/charge path and the admin BookingsPage). Three of the four names in the cleanup list are never written by any code; the one that matters is missing. After a production test-run cleanup, orphaned charge docs survive their deleted parent bookings and flow into Reports, which aggregates `collectionGroup("charges")` — test revenue polluting live reports is exactly what ETR exists to prevent. The staging-reset path in the same file already uses the correct list, which is how the discrepancy escaped review. **Fix:** add `charges` to the `handleDeleteTestRun` subcollection list (and verify/prune the never-written names), add a regression test asserting charge docs are deleted, rebuild + commit the bundle (pairs with AUD-01).

**SEV-3:**

- ⬜ **AUD-03 — 12 guest API tests broken by intentional behavior changes; merged commits claimed green suites.** Bisected: PRC-07's method-aware transaction-reference enforcement broke 8 tests in `bookings-payment.test.ts` + 1 in `bookings-create.test.ts` the moment it landed (they post digital payments with no reference, which now correctly 400s), and the UCO unpaid-checkout gate broke 3 in `bookings-checkout.test.ts` (checkout with a balance now requires a reason). The handlers are correct per the PRC/UCO specs — **fix the tests to match the new behavior**, not the handlers. Process note: both commit messages claimed passing tests, so the full guest API suite was not run before merge; run the complete workspace test suite before merging to `dev`.
- ⬜ **AUD-04 — 5 pre-existing failures in `bookings-member-discount.test.ts` (predate this batch).** Already failing before the 2026-07-15 work: `handleCreateBooking` returns 400 before the member-token verification the tests assert on. Likely a stale mock or date-dependent payload; diagnose and fix separately from AUD-03.

**SEV-4 (polish):**

- ⬜ **AUD-05 — Verify-and-record idempotency key can swallow a legitimate duplicate payment.** `handleVerifyAndRecordPayment` treats a payment matching amount + method + transactionReference as a replay. For reference-free methods (e.g. cash configured with no required reference), a genuine second installment of the same amount is silently absorbed as a replay and never recorded. Low likelihood in the verify-proof flow (mostly digital + referenced), and the normal cash path (`add-payment`) uses a client-supplied payment ID. **Fix option:** have the client send an idempotency token (as `add-payment` does) instead of inferring from field equality.
- ⬜ **AUD-06 — Test-run cleanup stale-lock window (5 min) may be shorter than a large cleanup.** A second admin retrying after 5 minutes can reclaim the lock while the first run is still deleting. Deletion queries are idempotent so the worst case is duplicate work and a confused progress checkpoint, not data loss. **Fix option:** heartbeat the lock (refresh `cleanupStartedAt` per page) or raise the timeout to match the staging reset's 30-minute window.
