...
- [x] **Refund workflow (FIN-03 + CRL-01)** — Admin-only focused modal appends `type: "refund"` with negative amount, required method/reason, `approvedBy`, timestamp via authenticated `/api/bookings/add-refund`; server rejects refunds above current net collected; refund entries immutable, rendered in red. **Idempotent (CRL-01, 2026-08-01)** — the client preallocates a `refundId` (a Firestore-generated `bookings/{id}/payments/` doc ID) and holds it across uncertain responses, so a retry after a network blip or a closed tab replays the original commit instead of appending a second refund entry. The same `refundId` with different amount/method/reason/transactionReference is rejected as a 409 conflict; the held ref is cleared on success or 409 so the next intentional submit mints a fresh ID.
...
- **Server** — `POST /api/bookings/confirm-with-balance` (rate-limited 30/min/IP, staff-authenticated). Atomic Firestore transaction: re-reads booking + payments + charges + add-to-bill store orders, validates `status === "payment-uploaded"`, computes the charge-inclusive balance, reads `hotelConfig.unpaidCheckoutApprovalThreshold` inside the transaction, enforces the same `front-desk`/`admin` gate as unpaid-checkout, and stamps the four `confirmedWithBalance*` fields + flips `status` to `confirmed` + writes `paymentConfirmedAt` + `handledBy`. Side-effects: `booking-confirmed-with-balance` email to the guest (room type only, never room number — `confirmed` is not yet `checked-in` per the room-number-visibility rule), and a `booking` notification for the bell.
