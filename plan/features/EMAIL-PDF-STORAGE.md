...
- [x] **Booking-detail table renders "Booked on" + "Originally for" rows** (CRL-08 follow-up, decision #213, shipped 2026-08-11) — the shared `bookingRows(booking)` template in `guest-app/server/handlers/email.ts` appends `${row("Booked on", formatDate(booking.bookedOn))}` + `${row("Originally for", formatDate(booking.originallyFor))}` to BOTH the reservation-scope + the single-booking branch so the two templates never drift. The values are populated by `buildReservationEmailView` (reservation-scope) or by the new `sendBookingTrigger` enrichment (single-booking + legacy pre-MRB-01). "Originally for" is suppressed when the helper returns `null` (the booking has never been rescheduled). The "Booked on" row always renders when the booking has a `createdAt`. All 9 email templates that go through `bookingRows` carry the two rows: `booking-submitted`, `payment-confirmed`, `booking-confirmed`, `checkin-reminder`, `booking-cancelled`, `booking-cancelled-reservation`, `booking-rescheduled`, `discount-rejected`, `early-checkin-request`. The `booking-confirmed-with-balance` template also gets the rows via the same `bookingRows` call. Tests: `guest-app/tests/api/crl-08-reschedule-snapshot-refresh.test.ts §CRL-08 confirmation emails + receipt PDF — surface` (5 source-text guards on the template + the enrichment + the server-side PDF + the admin receipt).
...
- [x] **Booking-date surface (CRL-08 follow-up, decision #213, shipped 2026-08-11):** the server-side receipt PDF (`generateReceiptPdf` in `guest-app/server/handlers/email.ts`) and the admin receipt PDF (`printBookingReceiptPDF` in `admin-app/src/pages/BookingsPage.tsx`) both render "Booked on" + "Originally for" lines in the booking-details / Stay card. "Booked on" = the booking's `createdAt` (or the reservation header's `createdAt` for new reservations); "Originally for" = the create-time scheduled check-in (the reservation header's `checkIn` per MRB-14 immutability for new reservations, or `rescheduleHistory[0].fromCheckIn` for legacy pre-MRB-01). The "Originally for" line is suppressed when the original equals the current check-in (the booking has never been rescheduled). The two values are precomputed at the server (single + reservation-scope email views) and at the admin parent (the receipt Stay card) via the `getBookedOnDate` + `getOriginallyForCheckIn` helpers; the PDF generator + the receipt Stay card render the friendly "Aug 7, 2026" string. Same shape across the booking-confirmed email attachment + the admin "Print receipt" button — the staff + the printed PDF + the email attachment read the same metadata.
