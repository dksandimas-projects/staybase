...
- [x] **Environment banner (ETR-22, 2026-09-19, v0.276.0)** — every server-rendered email prepends one of two stacked callouts at the top of the white content card. **Test-run callout** (🧪 indigo): fires when the source booking has `isTestData: true` AND the booking doc carries `testRunName` + `testRunEnvironment` (stamped onto each child at create time by `handleCreateBooking` + `handleCreateWalkin`). Copy falls back honestly: full descriptor → name-only → env-only → "active test run." **Staging callout** (⚠️ amber): fires on every email when `isStagingProject()` returns true (gated by `FIREBASE_PROJECT_ID` × `STAGING_ALLOWLIST_PROJECT_IDS` — the same allowlist the staging reset uses). Both callouts stack with test-run on top when both apply. Lives in `guest-app/server/handlers/email-banner.ts` so the helpers can be unit-tested without dragging in firebase-admin / resend / jsPDF; `emailLayout` accepts `bannerHtml?: string` and renders it as the first child of the content cell (above the intro paragraph). All 27 email template call sites pass `bannerHtml`. **Preview auto-detect (ETR-22.9)** — `handleEmailPreview` enriches `mockBooking` into `enrichedMockBooking` so the Settings → Email Config tab shows the same banner the guest would see. Two input paths: body opt-in (`{ isTestData, testRunName, testRunEnvironment }`) for one-off overrides, or auto-detect (query `testRuns` for the most recent active run in the current deployment environment). **Subject-line prefix (ETR-22.10)** — mirrors the body banner so the environment + test-run state is visible in inbox list views + push notifications without opening the email. `sendEmail` now prepends `[PROD TEST] ` / `[STG] ` / `[STG TEST] ` (production + no test-run = no prefix). The 6 booking-aware trigger functions pass the booking view's test-run fields through as the new 5th `banner` arg; staging auto-detects inside `sendEmail` so non-booking triggers (store orders, voucher, verification) get the staging prefix on staging for free. Suppressing the email sends entirely for test runs is deferred to a follow-up ticket — the banner is the production-side analog ("you got a real email but it's just a test"). See `plan/features/ENVIRONMENT-TEST-RESET.md §Email environment banner (ETR-22)` for full spec, out-of-scope list, and the test coverage breakdown.
