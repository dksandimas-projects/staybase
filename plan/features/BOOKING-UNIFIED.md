...
- **BAR-02 ✅ shipped 2026-08-08 on `refactor/bar-02-derive-reservation-counters` (per decision #203, v0.264.18 → v0.264.19).** The 5 denormalized counter fields + the `paymentStatus` aggregate mirror on the reservation header are no longer written. Consumers derive them at read time via `deriveReservationCounters(children)` + `computeReservationAggregatePaymentStatus(childStatuses)` in `shared/utils/bookingFolio.ts`. The MRB-15-03 counter-proliferation is gone (the counters can no longer drift because they no longer exist). The FOL-05 sibling-flip pass is unchanged (per-child status transitions are the real state mutation). The `Reservation` type drops the 5 counter fields + `paymentStatus` to optional `?` for back-compat reads of pre-BAR-02 reservations. See `plan/docs/DECISIONS-FEATURES.md #203` for the full record.
- **BAR-03 ✅ shipped 2026-08-08 on `refactor/bar-03-reservation-scope-transition-helper` (per decision #204, v0.264.19 → v0.264.20).** The FOL-05 sibling-flip pass is now shared between the 3 payment handlers via the new `applyReservationScopePaymentTransition` + `preReadSiblingChildren` helpers in `guest-app/server/handlers/reservationScopeTransition.ts`. Each handler passes a per-handler `rule.decide` callback (verify / add: `payment-confirmed` + 3 stamps; reject: `pending` + 5 stamps); the helper handles the per-sibling `transaction.update` + the post-update statuses array + the reservation header heartbeat. The per-child status transitions are byte-equivalent to pre-BAR-03; the FOL-03 reads-before-writes invariant is preserved; the BAR-02 header heartbeat is preserved. Future FOL-06 (apply-discount) + FOL-07 (partial refund) will call the helper with a per-handler rule (~10 lines per handler instead of ~50). See `plan/docs/DECISIONS-FEATURES.md #204` for the full record.
