...
- [x] **Early check-in requests section** (EC-01, 2026-08-21; empty-state update feat/early-checkin-empty-state, 2026-08-22) — list of bookings with `earlyCheckIn.status === "requested"`, guest name, requested time, optional guest notes, room number, **Approve** + **Decline** buttons per row; Decline opens the shared `ConfirmForm` with a required reason; container always renders (mirrors the pending-payments summary tile pattern — neutral gray tone + "No pending requests" copy when empty so staff see the section is empty, not broken); sibling to + styled like the pending-payments card
...
- [x] **Approved early check-in badge on Today's Check-Ins** (feat/early-checkin-arrivals-badge, 2026-08-21) — bookings whose `earlyCheckIn.status === "approved"` render an inline green pill in the check-ins row showing `early <time>` with a `Check` glyph (lucide); gated on `approved` only — declined + pending requests live in their own widgets. The badge text follows the existing `confirmedTime || requestedTime` precedence (the same rule used in the guest email confirmation + StaysPage + RewardsPage) so when staff override the guest's requested time, the badge reflects the staff override. `data-testid="early-checkin-arrival-badge"` is per-booking for the e2e harness; the `title=` carries the full ISO-style text for hover-tooltip.
...
- [x] No pending early check-ins — widget container still renders with neutral gray tone + "No pending requests" copy; body row list is hidden so empty state is visible-but-not-noisy (per feat/early-checkin-empty-state, 2026-08-22; reverses the prior "hide entirely" decision so staff can distinguish empty from broken)
