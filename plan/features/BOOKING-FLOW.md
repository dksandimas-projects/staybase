...
- [x] **Special requests — staff-closed-loop flow (per feat/special-requests-redirect commit 78a79f7 + feat/staff-special-requests-capture 2026-08-21):** the public `/book` form no longer collects a free-text special requests field. A redirect card with the per-hotel contact override (Settings → Hotel Info → `hotel.config.ts` fallback) replaces the textarea — email + phone read from `usePublicSiteContent` (same source-of-truth chain as ContactPage + Footer). The redirect is rendered on the confirmation page too (between the email alert and the early check-in section) so the guest sees the "did you forget?" prompt at the moment they're most likely to remember a special need. The front desk captures inbound requests (from email or phone) into the booking doc via three staff surfaces: (1) the booking drawer's "Special requests" editor in `BookingDrawerWorkspace.tsx` (existing bookings, inline edit with last-edit metadata), (2) the admin walk-in modal, and (3) the calendar-create modal — both create paths forward the captured value through `addWalkinBooking` to the walk-in API endpoint. The intercom amber banner (`IntercomChatPanel.tsx`) keeps surfacing the stored value for any in-flight bookings that already carry a non-empty `specialRequests`. The calendar grid shows a small amber icon + hover-tooltip on cells whose booking has a non-empty request so the next shift sees it at a glance. The `specialRequests` field stays on the `Booking` schema for back-compat, plus two metadata fields (`specialRequestsUpdatedAt` + `specialRequestsUpdatedBy`) that the staff write paths stamp in the same transaction.
...
- [x] **Member autofill (timing contract — `fix/booking-autofill-member-profile-race` 2026-08-22):** when the guest is signed in as a Spark Rewards member, Step 2 pre-fills `firstName` / `lastName` / `email` / `phone` from `memberProfile`. Two surfaces ship the autofill — the public `/book` flow (`BookingPage.tsx`) and the corporate `/corporate/book` flow (`CorporateBookingPage.tsx`). The lock pattern (readOnly + `updateGuestDetail` short-circuit) is unchanged from the prior PRs (`feature/booking-autofill-member-name`, `feature/booking-autofill-member-email`).
  - **Two-stage timing:** the `guestDetails` initializer runs once on mount (synchronous). `memberProfile` arrives asynchronously via `onAuthStateChanged` + Firestore `onSnapshot` (typically 200–500ms after mount). The initializer always sees `memberProfile = null` and falls through to `""`. A `useEffect` in each page re-applies the autofill when `memberProfile` lands (or changes). The effect deps are `memberProfile?.isMember`, `memberProfile?.fullName`, `memberProfile?.email`, `memberProfile?.phone`, and `searchParams` (URL params keep precedence for the Step 4 → re-book flow).
  - **"Only fill empty fields" guard:** the effect uses `prev.firstName || derivedFirst` etc. so a guest who types into a field before the snapshot lands does NOT have their value overwritten. Without this guard, a guest on a slow connection (or with a slow Firestore cache) would see their manual input erased when the snapshot fires. URL params keep precedence (the corporate personal-pay path can pin a billing-email override via `?email=`).
  - **Regression net:** the existing source-text guards (`tests/api/booking-autofill-member-name.test.ts`, `booking-autofill-member-email.test.ts`, `booking-autofill-member-name-corporate.test.ts`, `booking-autofill-member-email-corporate.test.ts`) stay green — the new `useEffect` is additive, the existing initializer + readOnly + short-circuit pattern is preserved.
  - **Future refactor:** the two pages share an identical `useEffect` + `setGuestDetails` re-sync block. A `useMemberAutofill(setGuestDetails)` hook in `guest-app/src/hooks/` would deduplicate. Not in this PR — keep the two pages in lock-step for now.
