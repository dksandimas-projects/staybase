...
- [x] **Special requests redirect (per feat/special-requests-redirect, 2026-08-21):** the public `/book` form no longer collects a free-text special requests field. A redirect card with the per-hotel contact override (Settings → Hotel Info → `hotel.config.ts` fallback) replaces the textarea — email + phone read from `usePublicSiteContent` (same source-of-truth chain as ContactPage + Footer). The redirect is rendered on the confirmation page too (between the email alert and the early check-in section) so the guest sees the "did you forget?" prompt at the moment they're most likely to remember a special need. Front desk still captures special requests at the counter via the admin walk-in / calendar create paths (the `WalkinGuestDetailsSchema.requests` field is unchanged). The intercom amber banner (`IntercomChatPanel.tsx`) keeps surfacing the stored value for any in-flight bookings that already carry a non-empty `specialRequests`. The admin receipt PDF no longer renders a "Special Requests" section. The `specialRequests` field stays on the `Booking` schema for back-compat.
