...
- **NC-02d (⬜, follow-up)** — add an **emulator-based rules test** (write as staff → mark-own-read allowed, removal/foreign-UID denied) and/or wire `firebase validate` into CI. Grep-based rules tests cannot catch an invalid-but-present rule (that's exactly how NC-02c shipped).
