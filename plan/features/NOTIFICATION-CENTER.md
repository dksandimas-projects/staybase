...
**Partially fixed — follow-up open:**
- **NC-02 (SEV-4) 🔄 — `readBy` rule scope.** The rule let any staff member rewrite the whole `readBy` map, not just their own UID. **Tightened** to bound the key set to `existing ∪ {my UID}` and require the writer's own value to be a timestamp — this closes UID *injection* and junk-value injection. **But it does not close removal/forgery of others' existing entries** (see NC-02b): `readBy = {me: ts}` is a valid subset, so it silently wipes another staff member's read entry; `readBy = {bob: <other ts>, me: ts}` passes because Bob's key pre-exists and only the writer's own value is checked.
- **NC-02b (SEV-4) ⬜ — close the removal vector.** Add `&& resource.data.readBy.keys().hasOnly(request.resource.data.readBy.keys())` to force every existing key to survive, so the key set can only grow by the writer's own UID. Value-tampering on others' existing entries stays reachable (Firestore rules can't loop over map values to compare each unchanged) and is **knowingly accepted** at this severity (trusted staff; worst case a wrong unread badge on a colleague's session).
