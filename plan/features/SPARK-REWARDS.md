...
- [x] **Front-desk manual link (Spark Rewards audit 2026-07-18 MED-3, build variant)** — **Shipped 2026-07-29** (decision #135 build variant, branch `fix/spark-rewards-med-3-link-booking`). New server endpoint `POST /api/members/link-booking` (`handleLinkBookingToMember` in `guest-app/server/handlers/members.ts`) — admin-only (front-desk 403), Zod-validated `{ memberUid, bookingId, reason (1..500) }`, 10/min/IP rate limit (mirrors `manual-adjust` + `set-active`). Server transaction re-reads the member + the booking, refuses cancelled bookings (400), test-run bookings (400), and bookings already linked to a different member (409 — no silent overwrite, no unlink from this surface), then sets `memberId` + `linkedByStaff` + `linkedAt` + `linkedReason` on the booking and writes an audit row under `bookings/audit/records/{bookingId}-link-{timestamp}` (mirrors the erasure audit shape from decision #49). Re-linking a booking to the same member is idempotent (no booking update, audit row still written — `alreadyLinked: true` in the response). The admin member drawer (`admin-app/src/pages/MembersPage.tsx`) gains a **Link Existing Booking** form below the points adjustment form, with the member's email surfaced in the helper copy so the staff can confirm the different-email context before linking. Toasts: success on a fresh link, info on a re-link (alreadyLinked). **Tests** — 14 server tests in `guest-app/tests/api/members-link-booking.test.ts` (auth + method + schema + lookup + booking-state + idempotency + audit row + Admin SDK posture) + 18 source-text tests in `admin-app/src/__tests__/med-3-link-booking.test.ts` pinning the route wiring, the handler contract, the AdminContext function, and the drawer form. 930/930 admin + 544/544 guest tests green, typecheck clean. The guest self-service prompt in the next entry remains the unbuilt half of MED-3.
  ...
  > **⚠ MED-3 build-variant follow-ups needed (operator-reported 2026-08-20, two known gaps — NOT YET SHIPPED).** The original MED-3 ship note above predates MRB-01 (decision #159, 2026-08-02), so it does not anticipate the reservation-scope booking model or the staff typing patterns that surface in production. Two concrete gaps surfaced post-ship. **(G1) Lookup-by-ref mismatch.** The handler's lookup is `adminDb.collection("bookings").doc(bookingId).get()` (`members.ts:808`) — a doc-id lookup, not a field query. The form placeholder says "e.g. SPK-2026-0142 or booking doc id" (`MembersPage.tsx:442`), which invites staff to paste the **human booking reference** (`bookingRef` field, e.g. `SPK-YYYYMMDD-NNNN`). The canonical "look up by `bookingRef`" pattern lives at `guest-app/server/handlers/email.ts:932` and `guest-app/server/handlers/bookings.ts:6053`: `adminDb.collection("bookings").where("bookingRef", "==", input).limit(1)`. When the staff pastes a reference, the doc-id lookup returns `{ exists: false }` → handler throws `Error("Booking was not found.")` → handler's catch (`members.ts:894-901`) string-maps `"was not found"` to **400** with the verbatim "Booking was not found." message. The toast surface is "Booking link failed — Booking was not found." which reads as the staff having typoed rather than as "the server expected a doc id, you gave it a ref." For MRB bookings the placeholder should also accept `R-YYYYMMDD-NNNNN` (the `reservationRef`); the resolver needs to try the booking-ref lookup first and fall through to a reservation-id `reservations/{ref}.get()` lookup before falling through to a doc-id lookup (the doc-id path stays for backwards compat with the legacy null-`reservationId` bookings pre-MRB-01). **(G2) MRB-aware sibling write.** The transaction writes `memberId` on a single `bookings/{id}` doc (`members.ts:849-855`). For an N-child-room booking with `bookingData.reservationId` set, **only one child room appears in the member's My Stays list** — the other siblings stay orphaned (the read-side counterpart at `handleListMemberStays` `members.ts:192` queries `where("memberId", "==", uid)` on the flat `bookings` collection, with no expansion). The audit row at `members.ts:863-875` also omits `reservationId` / `reservationRef` — anyone reconciling an MRB link later has no way to know whether the staff linked one child or the whole reservation. The desired semantics: "linking a member to a stay means linking them to **the whole reservation** (or the single legacy booking if no `reservationId` exists)". The same problem lurks on the only unlink path (the booking-drawer `memberId` edit) — out of scope for MED-3 but worth flagging in this note. **Status** — spec'd here in plan/features/SPARK-REWARDS.md; tracked as a roadmap ticket under `plan/project/ROADMAP.md §MED-3 build-variant follow-ups (lookup-by-ref + MRB-aware sibling write)`. **Not implemented yet** — see the roadmap ticket for the proposed scope + the pre-flight data check that determines whether a backfill migration is needed.
