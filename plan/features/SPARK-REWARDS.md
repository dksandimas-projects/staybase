...
- **MED — "Different email" booking reconciliation is unbuilt despite the checklist marking it done.** The §Edge Cases entries "Google account email differs from booking email — after sign-in, prompt to link" and "manual link by front desk from Member detail drawer" are both marked `[x]` but neither is implemented: there is no guest self-service linking prompt in `guest-app/src`, and the admin member drawer (`MembersPage.tsx`) has no booking→member link action. A guest whose account email differs from an earlier anonymous booking's email won't get that booking auto-linked; the workaround is booking lookup via `/my-booking`. Build the front-desk manual-link path (smaller surface) or re-mark those lines "Deferred."
...
- **LOW — Erasure placeholder email `erased@invalid` bypasses the send-skip guard.** Anonymized bookings get `guestEmail: "erased@invalid"` (`members.ts`), but `sendEmail`'s skip guard (`email.ts`) only matches `@example.invalid`. A stray trigger would attempt delivery to the invalid address (caught, bounces) instead of a clean skip. Align the placeholder domains.
