...
- [x] Guest booking confirmation (Step 4): new "Request early check-in" section on `BookingConfirmPage.tsx`, rendered only when `isRewardsMember && earlyCheckInEnabled && bookingStatusAllowsEarlyCheckIn` (the status allowlist `[payment-uploaded, payment-confirmed, confirmed]` mirrors the server gate in `guest-app/server/handlers/email.ts` so the button only appears when the click would succeed — see `fix/early-checkin-payment-uploaded-allowlist` 2026-08-21). Disclaimer copy next to the button ("**not guaranteed** — subject to approval by our team, and you'll receive an email once we approve or decline") + repeats inside the modal. Single-booking flow (URL params carry the bookingRef) — submission uses the existing `/api/email` path, no new server route.
- [x] Modal close on success (per `fix/early-checkin-modal-success-state` 2026-08-21): after a successful submit the modal swaps the form for an inline success panel — a `CheckCircle2` glyph, "Request sent" copy, and a single Done button that closes the modal. The X close button at the top of the modal header continues to work in both states. `earlyCheckInSent` resets inside `closeEarlyCheckInModal` so re-opening the modal starts on the form, not on a stale success panel. (Pre-fix the modal stayed open showing the same form after a successful submit — the guest had no feedback that anything happened. The bug was masked by `earlyCheckInSent` being set but never read by the JSX.)
