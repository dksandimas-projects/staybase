...
- [ ] Points redemption: POST `/api/members/redeem-points`; API transaction validates member balance and redemption rate, updates booking total and locked rate breakdown together, deducts points, and writes points history
- [ ] Undo points redemption: POST `/api/members/undo-redemption`; admin-only and only while booking status is `confirmed`; restores booking total and removes the points deduction from the locked breakdown in the same transaction
