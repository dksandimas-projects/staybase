...
- **Cancel routes through `scope: "reservation"`** (per `plan/docs/DECISIONS-FEATURES.md #166` / MRB-13). The cancel modal copy states the room count ("This will cancel all N rooms in your reservation.") + a per-room list so the guest can verify before confirming. The server's `handleCancelBooking` honours the scope selector; the existing `ref + (email | token)` credential is unchanged.
