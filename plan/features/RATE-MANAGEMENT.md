...
- [x] Save button writes the full rate matrix in a single batched `saveRoomTypes(nextRoomTypes)` call (one Firestore write covering every type); toast on save: "Rates saved — Rate matrix updated for all room types"
...
- [x] **Rate update: single batched `saveRoomTypes(nextRoomTypes)` write** — the full next array is computed once in `handleSaveRates` and written once via `saveRoomTypes` in `AdminContext`. The previous shape fired N concurrent `updateRoomType` calls (one per type), each reading the same render-time `roomTypes` snapshot and writing the whole array back via `setDoc(..., { merge: true })`. Merge of an array field replaces it wholesale, so the last write to land won and every other type's edit was silently dropped (RTS-01, see ROADMAP §Phase 12). The new contract is one write, no race. Per-field state still flows through `prices` (form buffer) and `dirtyRateFields`; only the persist call is bulk. Single-item updates from the Settings → Room Types Edit modal still go through `updateRoomType` (singly, no fan-out) and remain safe. See `plan/docs/GOTCHAS.md` Firebase section for the reusable lesson.
