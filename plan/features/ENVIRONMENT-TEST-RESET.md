...
- ✅ **ETR-22.9 — Preview auto-detects environment + active test run.** `handleEmailPreview` enriches `mockBooking` into `enrichedMockBooking` so the rendered template matches what a guest would actually see in the current deployment. Two input paths: **(a)** body opt-in — `{ isTestData: true, testRunName: "...", testRunEnvironment: "staging"|"production" }` for one-off overrides; **(b)** auto-detect — queries `testRuns` for the most recent active run in the current deployment environment (`isStagingProject()` → `"staging"` or `"production"`), reads its `name` + `environment`, stamps them onto the enriched mock. Falls back to no test-run banner if the read fails or no active run exists — the preview never breaks. The 14 booking-template switch cases + 3 early-checkin + 2 staff templates use `enrichedMockBooking`; non-booking templates (corporate, contact, voucher, verification, store-order) keep their typed mocks (they read `environmentBanner({})` so the staging banner still fires on staging). One extra Firestore read per preview (admin-only, staff-auth-gated, scoped query) — negligible cost. Tests: 4 source-text guards + 4 integration tests in `email-environment-banner.test.ts` + `email-preview.test.ts` (default = no banner, staging = staging banner, body opt-in = test-run banner with full descriptor, body opt-in with name only = "test run \"name\"").
- ✅ **ETR-22.10 — Subject-line environment prefix.** Mirrors the body banner so the environment + test-run state is visible in inbox list views + push notifications (no need to open the email). New `subjectPrefix(state)` helper in `email-banner.ts` produces a deterministic prefix:

  | Deployment | Test-run | Prefix |
  |---|---|---|
  | Production | no | *(none)* |
  | Production | yes | `[PROD TEST] ` |
  | Staging | no | `[STG] ` |
  | Staging | yes | `[STG TEST] ` |

  `sendEmail` now accepts a 5th `banner?: { isTestData?, testRunName?, testRunEnvironment? }` arg. Staging auto-fires inside `sendEmail` via `isStagingProject()` (the canonical allowlist). The 6 booking-aware trigger functions (`sendBookingTrigger`, `sendBookingConfirmedWithBalanceTrigger`, `sendStaffNewBookingTrigger`, `sendStaffNewPaymentTrigger`, `sendEarlyCheckinRequestTrigger`, `sendEarlyCheckinResolveTrigger`) read the booking view's stamped test-run fields and pass them through. The DLQ `failed_emails` row uses the prefixed subject so a failed-send audit stays consistent with what would have been delivered. Non-booking triggers (store orders, voucher, verification, staff refund review) intentionally omit the banner arg — staging prefix still fires for them on staging deployments, but no test-run prefix (store orders don't stamp `isTestData` today). Tests: 12 unit + source-text in `email-environment-banner.test.ts` (4 states × production/staging, multi-project allowlist edge cases, `subjectPrefixFromBooking` null-safety, `sendEmail` signature, every booking trigger constructs and passes the banner arg).
