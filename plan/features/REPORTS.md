...
- [ ] **CRL-07/08 cancellation liability queue + refund-state email** — the destructive cancel now stamps a durable `cancellationLiability` snapshot onto the cancelled entity (reservation header for reservation-scope cancels + new-path N=1, booking doc for per-child + legacy null-`reservationId` cancels). The Reports query reads the stored snapshot (per the spec: "processed refund (derived from immutable ledger entries)") + sums the refunds subcollection on the fly to project one of five states (`not-required` / `retained` / `pending-processing` / `partially-processed` / `processed`) via the pure `computeCancellationLiabilityState` helper. The pending liability queue (per the CRL-07 spec body) surfaces pending count, pending amount, partials, age, processed total, and retained cancellation revenue; exports + Daily Close continue to derive actual cash movement from the payment ledger, never from `approvedAmount`. The refund-state email (CRL-08) lists target rooms, revised total/balance, policy refund, retained amount, and current processing state. Spec: `plan/docs/DECISIONS-FEATURES.md #173`. Behavioural tests + staging rehearsal follow in CRL-09.
