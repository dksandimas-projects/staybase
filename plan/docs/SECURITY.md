...
- **Staff operational updates:** Authenticated staff/admin may directly update only `guestIdPhotoUrl`, `guestRegistration`, `breakfastSelections`, `breakfastServed`, `discountVerified`, `discountVerifiedBy`, `discountRejected`, `notes`, `specialRequests`, `handledBy`, and `updatedAt`. The `specialRequests` field is preserved for back-compat (in-flight bookings written before `feat/special-requests-redirect` shipped on 2026-08-21 may still carry a non-empty value); the public `/book` form no longer collects the field, but the admin walk-in / calendar create paths still post it via the `WalkinGuestDetailsSchema.requests` input (the front desk types it on the guest's behalf at the counter) and the intercom amber banner surfaces it to the staff. (Per 2026-07-24 `refactor/unify-payment-reference-fields`: the previous `paymentReferenceNumber` allowlist entry was removed — staff no longer write a top-level reference; the canonical reference lives on each `bookings/{id}/payments/{paymentId}` entry's `transactionReference`, which is append-only at the rules level.)
