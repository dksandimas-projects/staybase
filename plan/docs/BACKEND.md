...
**Per MRB-02.x corporate (2026-08-02, per decision #164) — corporate `/corporate/book` preallocation + fingerprint fix:** the corporate flow goes through the same `/api/bookings/create` endpoint as the public `/book` flow, but with a different body shape (additional `corporateCode` / `corporateFlatRate` / `companyName` / `designation` / `companyAddress` / `purposeOfStay` / `preferredBillingArrangement` fields). MRB-02.x corporate has two parts: (1) **the corporate client preallocates `reservationId`** via the shared `generateReservationId()` helper (`useState` lazy init on `CorporateBookingPage.tsx`, same pattern as `BookingPage.tsx`) so a retry-after-uncertain-response reuses the same id. (2) **the server's fingerprint is corporate-aware** — the public create handler's function-scope `reservationRequestFingerprint` block now derives `isCorporateIntent = Boolean(corporateCode) || corporateFlatRate === true` and uses that flag for the fingerprint's `source` + `isCorporate` + `companyName` fields. Pre-fix, the fingerprint used `Boolean(corporateCode)` for `isCorporate` + `source`, which gave `false` / `"online"` for the "Continue without code" path — a mismatch with the server-validated `isCorporate: true` / `source: "corporate"` the transaction stamps. The fix threads the intent flag through so the fingerprint's source + isCorporate + companyName match the stamped values for both corporate paths. The `companyName` is the body-entered name (the guest's stated company) — the fingerprint's purpose is to detect "different intent", and the guest's stated name IS the intent. A retry with a different stated name is a different intent (409); a retry with the same stated name is the same intent (replay). The fingerprint's `companyName` can differ from the server-stamped `companyName` (the doc's enforced name for the "with code" path), but the mismatch is the expected behavior — the fingerprint is the intent, the stamped value is the enforcement. Same `requestFingerprint` comparison on the reservation read still detects "different intent" correctly.

**Per MRB-06 Phase 1 (2026-08-02, per decision #159) — N>1 room assignment foundation:** the public create handler's auto-assignment loop generalizes from single-room to N>1 in one transaction. The body may include an optional `roomCount` (default 1, max 50). The inner candidates loop (the historical single-room assignment, sorted by `roomNumber` for deterministic assignment) is wrapped in a `Math.max(1, floor(roomCount))` outer loop. Each iteration picks the first non-conflicting candidate; once a room is assigned, it's pushed to `assignedRoomIds` and the next outer iteration can't pick it again (the same-room-twice guard). If the inner loop runs out of non-conflicting candidates before `roomCount` is satisfied, the entire transaction aborts — no partial write. The reservation header's `reservationRoomCount` reflects the N assignments. For N=1 (the default) the behavior is byte-equivalent to pre-MRB-06. **Phase 1 scope (this commit):** the schema + auto-assignment + header are wired. The N booking write loop (each assigned room gets its own `bookings/{id}` doc with per-room `roomId` + `roomNumber` + `reservationPosition`) + the group totals on the header (`totalPrice` = sum of per-room totals) are Phase 2 (a follow-up commit). For N=1 today, only the first room's booking doc is written (the historical behavior, byte-equivalent). For N>1 today, only the first room's booking doc is written — the other N-1 rooms are "assigned" but not "booked" (a known limitation documented in the response payload's `roomCount` field).

**Per MRB-06 Phase 2 (2026-08-02, per decision #159) — N booking write loop + group totals + N>1 response shape:** the public create handler's transaction's write phase now iterates over the N assigned rooms and writes a `bookings/{id}` doc for each. The first room uses the client's preallocated `bookingId` (the historical contract); the other N-1 rooms auto-mint fresh ids via `adminDb.collection("bookings").doc().id` (same pattern as the walk-in path's `generateReservationId` auto-mint — a follow-up can switch the client to preallocating N ids once the N>1 client surface lands). Each per-room variant spreads `newBooking` (the per-type template) and overrides the per-room fields (`roomId` + `roomNumber` + `reservationPosition` (1..N) + `reservationRoomCount` (always N) + `lookupToken` (per-booking; the MRB-04 follow-up refactors to a per-reservation token)). For N=1 (the default) the loop runs once, byte-equivalent to the pre-MRB-06 single write. The reservation header's group totals (`totalPrice` + `originalSubtotal` + `subtotal`) are the N-aggregate: `per-room value * assignedRooms.length`. The EXB-10 inventory check counts `totalExtraBeds = extraBedCount * assignedRooms.length` (not the per-room count) so N>1 reservations don't silently under-count the hotel's rollaway footprint. The success response echoes the first room's id + number in the legacy `roomId` / `roomNumber` fields (backward compat with the N=1 confirmation page) plus a new `rooms` array carrying ALL N assignments (`bookingId` + `roomId` + `roomNumber` + `reservationPosition` per room) so the N>1 confirmation can render the full group view. For N=1 (the default) `rooms` is a single-element array — byte-equivalent to the pre-MRB-06 single fields.

**Per MRB-04 Phase 1 (2026-08-02, per decision #159) — reservation folio summary types + subcollection rules + behavior-frozen helper:** the reservation-level folio moves from the booking-owned model (the legacy `bookings/{id}/payments` + `bookings/{id}/charges` subcollections) to the reservation-owned model (the new `reservations/{id}/payments` + `reservations/{id}/charges` subcollections, with the reservation header as the single source of truth for the reservation's money state). **Phase 1 (this commit)** ships: (1) the `ReservationPayment` + `ReservationCharge` + `ReservationFolioSummary` types in `shared/types/index.ts` — the payment entry is sign-aware (positive for a payment collected, negative for a refund — the CRL-01 negative-amount convention), the charge entry mirrors the existing `bookings/{id}/charges.voidOf` semantics, the summary carries the canonical balance invariant fields. (2) The subcollection rules in `firebase/firestore.rules` — these were already in place from MRB-01 (server-authoritative `reservations/{id}/payments` with staff read + server-only create + no client update/delete; staff direct write on `reservations/{id}/charges` with the per-creator + bounds + void semantics). No rules change required. (3) The behavior-frozen `getReservationFolioSummary` helper in `shared/utils/bookingFolio.ts` — the canonical resolver that reads from the new subcollections for new reservations, and falls back to the legacy `bookings/{id}/payments` + `bookings/{id}/charges` for null-`reservationId` bookings (the legacy adapter). Pure function (no Firestore calls). The helper enforces the canonical balance invariant `reservation balance == reservationTotal + chargesTotal − paymentsTotal` with a single-pass sign-aware sum so the invariant is preserved at the math level — no separate derivation that could drift. (4) The balance invariant is pinned by 8 property tests in `shared/__tests__/booking-folio.test.ts` (zero balance, positive balance, overpaid, refund contributes negative, charges add to the total, defensive coercion, source flag round-trip, randomized property check across 50 iterations). **Phase 1 scope (this commit):** the types + the helper + the rules shape. The actual payment write paths (the refactor of `handleAddPayment` + `handleVerifyAndRecordPayment` + `handleAddRefund` to write to the new subcollections for new reservations, with the legacy adapter for null-`reservationId` bookings) + the Receipt / Reports / FIN reconciliation endpoints reading from the new subcollections + the migration of legacy bookings to the new subcollections + the deterministic rounding-remainder test (remainder always lands on the lowest room position; cancelling that specific room writes a labeled "rounding adjustment" line on that room's audit record per MRB open-question Q2) are Phase 2+ follow-ups.

**Per MRB-04 Phase 2 (2026-08-02, per decision #159) — `handleAddPayment` writes to `reservations/{id}/payments` for new reservations:** the first payment write path moves from the legacy booking-owned subcollection to the reservation-owned subcollection. The handler reads `bookingData.reservationId` (the canonical MRB-01 linkage) and selects the subcollection via a single ternary: when non-empty, the payment record goes to `reservations/{reservationId}/payments/{paymentId}` (the reservation header is the canonical money source for new reservations); when null (legacy pre-MRB-01 bookings), the record stays at `bookings/{bookingId}/payments/{paymentId}` — byte-equivalent to the pre-MRB-04 behavior. The `String((bookingData as any).reservationId || "").trim()` defensive coercion collapses `null` / `undefined` / whitespace-only legacy values to `""` so the legacy branch is a clean ternary fallthrough. The payment record shape mirrors the subcollection selection: new reservations get the legacy `paymentRecord` spread with `reservationId: bookingReservationId` + `bookingId: bookingId` (the per-room attribution field — `null` for reservation-level payments, non-null when the staff ties a payment to a specific room's add-on); legacy bookings get the byte-equivalent historical `paymentRecord` shape (the legacy `OnsitePayment` interface). The transactional write is `transaction.create(newPaymentRef, recordWithReservation)` for both paths — only the `ref` and the `record` differ. The status transition on the booking doc (`payment-confirmed` when `fullyPaid && isConfirmableStatus` + the `settlesCheckedOutFolio` loyalty award logic) stays unchanged for both paths. The reservation header's `paymentStatus` mirror update for new reservations is **NOT in this commit** — it lands with MRB-04 Phase 3 (a follow-up that mirrors the booking's `paymentStatus` on the reservation header inside the same per-resaction transaction scope). The CRL-01 idempotency contract is preserved byte-equivalently — same `paymentId` + same amount + same method + same note + same reference → `idempotentReplay = true`; same `paymentId` with different fields → 409 "Payment ID has already been used for a different payment." (the same status code + message as the refund path). The 409 catch-block mapping is unchanged. **Phase 2 scope (this commit):** `handleAddPayment` only. **Out of scope here, deferred to Phase 2.x+ follow-ups:** refactor `handleAddRefund` to write to the reservation subcollection (the design decision on whether to use a `refunds/` subcollection or negative-amount entries on `payments/` follows MRB-04 Phase 2.x) with the legacy adapter; refactor `handleVerifyAndRecordPayment` to write to the new subcollection (the same pattern); the reservation header's `paymentStatus` mirror update (MRB-04 Phase 3); the Receipt / Reports / FIN reconciliation endpoints reading from the new subcollections; the migration of legacy bookings to the new subcollections; the deterministic rounding-remainder test (per MRB open-question Q2).

**Per MRB-04 Phase 2.x (2026-08-02, per decision #159) — `handleAddRefund` + `handleVerifyAndRecordPayment` write to the reservation subcollections (the "Both, as separate paths" design):** the remaining payment write paths complete the reservation-level folio migration. The design picks the "Both, as separate paths" option (per the design call): **`reservations/{id}/refunds` is the canonical refund source** AND the helper `getReservationFolioSummary` ALSO reads `reservations/{id}/payments` for any negative-amount entries (belt-and-suspenders, catches edge cases like legacy CRL-01 backfills or admin pre-recordings). The writer only writes to `refunds/`, so the two arrays are disjoint in normal operation. The **new `ReservationRefund` type** in `shared/types/index.ts` is parallel to `ReservationPayment` but refund-specific (admin-only, requires `approvedBy` + `reason` + `method`; the canonical refund record shape is `{ id, reservationId, bookingId | null, amount: -|amount|, method, reason, transactionReference | null, approvedBy, recordedBy, recordedAt }`). The **new rule** `match /reservations/{reservationId}/refunds/{refundId}` is server-only create, staff read, no client update/delete (same shape as the existing `payments/` subcollection rule). The **`handleAddRefund` refactor:** subcollection selection reads `bookingData.reservationId`; when non-empty the refund record goes to `reservations/{reservationId}/refunds/{refundId}` (canonical), when null it stays at `bookings/{bookingId}/payments/{refundId}` (the historical CRL-01 contract — refunds are negative-amount entries on the booking's payments subcollection). **Net collected (the dual-read pattern):** for new reservations the handler reads BOTH `reservations/{id}/payments` (positive) AND `reservations/{id}/refunds` (negative) to compute net collected — `netCollected = netPositivePayments + netRefunds`. The two sums combine to the sign-aware net — the same math the helper `getReservationFolioSummary` uses. For legacy, the net is the historical sum of the booking's payments (which includes the negative-amount refund entries). Refund record shape: for new reservations the record carries `reservationId` + `bookingId`; for legacy it's byte-equivalent to pre-MRB-04. The preallocated `refundId` is still the doc id (CRL-01 idempotency contract preserved). The CRL-01 idempotency contract is preserved — same `refundId` + same `|amount|` + same method + same reason + same `transactionReference` → `idempotentReplay: true`; same `refundId` with different fields → 409. The 409 + 400 ("refund exceeds net collected") + 404 + 500 catch-block mappings are unchanged. The admin gate (`req.staff?.role !== "admin"`) is unchanged for both paths. The **`handleVerifyAndRecordPayment` refactor** follows the same pattern as the Phase 2 `handleAddPayment` refactor: subcollection selection (`reservations/{id}/payments` for new, `bookings/{id}/payments` for legacy); payment record shape (`recordWithReservation` spread adds `reservationId` + `bookingId` for new, legacy gets the byte-equivalent `paymentRecord` shape); the status transition (`payment-confirmed` when `fullyPaid`) stays on the booking doc for both paths. The CRL-01 idempotency contract is preserved — same `paymentId` + same amount + same method + same note + same `transactionReference` → `idempotentReplay: true`; same `paymentId` with different fields → 409 `PAYMENT_ID_CONFLICT`. The PRC-07 method-aware reference check (server-side config) is unchanged. The **helper update** (`getReservationFolioSummary` in `shared/utils/bookingFolio.ts`): the new optional `refunds: FolioReservationPayment[]` field (default `[]` keeps Phase 1 callers byte-equivalent) is summed into `paymentsTotal` alongside the existing `payments` array — `paymentsTotal = input.payments.reduce(...) + refunds.reduce(...)`. Refunds are negative entries (per the CRL-01 negative-amount convention); the sign-aware sum preserves the balance invariant at the math level. The balance invariant `reservation balance == reservationTotal + chargesTotal − paymentsTotal` is preserved for any mix of positive + negative entries on both subcollections. The legacy adapter passes `refunds: []` and supplies the negative-amount refund entries via the `payments` array — byte-equivalent to pre-Phase 2.x behavior. **Phase 2.x scope (this commit):** the two refactored handlers + the helper + the rules. **Out of scope here, deferred to follow-ups:** the reservation header's `paymentStatus` mirror update (MRB-04 Phase 3); the Receipt / Reports / FIN reconciliation endpoints reading from the new subcollections; the migration of legacy bookings to the new subcollections; the deterministic rounding-remainder test (per MRB open-question Q2).
...
**Per MRB-04 Phase 5A (2026-08-02, per decision #159) — admin drawer reservation folio:** the selected booking drawer resolves a reservation-linked stay from the reservation header total, canonical reservation payment/refund/charge snapshots, transitional payment/charge snapshots under every child booking, and qualifying room-billed store orders across those children. New charges are created under the reservation and require the selected child `bookingId`; reservation charge reversals must carry the same child attribution as their original. Transitional booking-owned charges are reversed in their original booking ledger rather than copied. The client preallocates payment/refund IDs from the canonical reservation subcollection for linked bookings, while legacy null-`reservationId` bookings keep the historical booking payments path. All listeners unsubscribe on selection changes and unmount. Receipt, Reports, FIN readers, and persistent header balance projections remain follow-up work.
