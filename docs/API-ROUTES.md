...
All email routes use Resend. Templates are defined server-side. See `features/EMAIL-PDF-STORAGE.md` for full email flow details.
...
Booking creation MUST use a Firestore transaction to prevent double-booking. See `features/AVAILABILITY-LOCKING.md`.
...
All public-facing routes (booking creation, voucher/code validation) apply a two-layer bot check before any business logic runs. See `docs/SECURITY.md §Bot & Spam Prevention` for full rationale.
...
See `docs/ENV-SETUP.md` for all required API environment variables including `RESEND_API_KEY` and Firebase Admin SDK credentials.
