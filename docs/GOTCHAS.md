...
- **Never read-then-write for booking creation** — always use a Firestore transaction. The client's prior system had real overbooking incidents. See `features/AVAILABILITY-LOCKING.md`.
...
- **Never hardcode room type strings** like `"single"` or `"executive"` in UI components — always iterate `config.roomTypes[]`. Hardcoded strings will be wrong for every non-Spark Inn client.
