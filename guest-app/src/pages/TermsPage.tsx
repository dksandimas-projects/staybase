...
export function TermsPage() {
  ...
  // Service body + version + last-updated are admin-editable
  // from Settings → Legal Content. The page reads from
  // `settings/websiteContent.termsBody` on mount; if a
  // non-empty custom body is set, render it (preserving
  // paragraph + list structure via `whitespace-pre-line`).
  // Otherwise fall back to the hardcoded 11-section body
  // below — same pattern as PrivacyPage's
  // privacyPolicyBody fallback. The version + last-updated
  // shown in the page header come from the Firestore doc
  // when available, else the deploy-time
  // `config.termsLastUpdated` (existing fallback).
    ...
    async function load() {
      try {
        ...
        if (!cancelled && snap.exists()) {
          ...
          if (typeof data.termsBody === "string" && data.termsBody.trim().length > 0) {
            setCustomBody(data.termsBody);
          }
          if (typeof data.termsVersion === "string" && data.termsVersion.trim().length > 0) {
            setCustomVersion(data.termsVersion);
          }
          if (typeof data.termsLastUpdated === "string" && data.termsLastUpdated.trim().length > 0) {
            setCustomLastUpdated(data.termsLastUpdated);
          }
        }
      } catch {
        // Fallback to deploy-time hardcoded body — already
        // rendered below.
      }
    }
    load();
  ...
  const lastUpdated = customLastUpdated || config.termsLastUpdated;
  const versionLabel = customVersion || VERSION;
        ...
        <article className="mx-auto max-w-4xl px-4 py-12 sm:px-6">
          <header className="mb-10 space-y-3">
            ...
            <div className="flex flex-wrap items-center gap-4 border-y border-gray-100 py-3 text-xs text-gray-500">
              ...
              {customVersion && (
                <span className="ml-auto rounded-full bg-primary/10 px-2.5 py-0.5 text-[10px] font-mono font-bold uppercase text-primary">
                  Admin-edited · v{customVersion}
                </span>
              )}
            </div>
          </header>
          ...
          {customBody ? (
            <div className="rounded-card border border-gray-200 bg-gray-50/30 p-6 sm:p-8 text-sm leading-relaxed text-gray-800">
              {/*
                Plain-text render — `whitespace-pre-line` preserves
                paragraph + list structure. No sanitization needed
                (no HTML interpretation). The admin editor rejects
                empty bodies client-side; the server-side Zod schema
                also enforces min(1) + max(50 KB).
              */}
              <pre className="whitespace-pre-wrap font-body text-sm text-gray-800">{customBody}</pre>
            </div>
          ) : (
