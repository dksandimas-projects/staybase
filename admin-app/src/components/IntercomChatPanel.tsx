...
export interface IntercomBookingSummary {
  guestName: string;
  bookingRef: string;
  checkIn: string;
  checkOut: string;
...
interface IntercomChatPanelProps {
  ...
  bookingSummary?: IntercomBookingSummary | null;
    ...
    >
      ...
      {bookingSummary && (
        <div className="shrink-0 border-b border-gray-200 bg-white px-4 py-3">
          <div className="grid gap-2 text-[10px] text-gray-500 sm:grid-cols-4">
            <div className="min-w-0">
              <p className="font-bold uppercase tracking-wider text-gray-400">Guest</p>
              <p className="truncate text-xs font-bold text-gray-900">{bookingSummary.guestName || "Guest"}</p>
            </div>
            <div className="min-w-0">
              <p className="font-bold uppercase tracking-wider text-gray-400">Booking</p>
              <p className="truncate text-xs font-semibold text-gray-800">{bookingSummary.bookingRef || "Walk-in"}</p>
            </div>
            <div>
              <p className="font-bold uppercase tracking-wider text-gray-400">Stay</p>
              <p className="text-xs font-semibold text-gray-800">
                {bookingSummary.checkIn || "--"} to {bookingSummary.checkOut || "--"}
              </p>
            </div>
            <div className="min-w-0">
              <p className="font-bold uppercase tracking-wider text-gray-400">Status</p>
              <p className="truncate text-xs font-semibold capitalize text-gray-800">{bookingSummary.status.replace("-", " ")}</p>
            </div>
          </div>
          {bookingSummary.specialRequests && (
            <p className="mt-2 line-clamp-2 rounded-lg border border-amber-200 bg-amber-50 px-2.5 py-2 text-[10px] font-medium leading-4 text-amber-800">
              {bookingSummary.specialRequests}
            </p>
          )}
        </div>
